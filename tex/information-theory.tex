...
\fist Fisher Information (Statistics \S\ref{sec:fisher_information})
...
\emph{Quantropy} -
\url{https://johncarlosbaez.wordpress.com/2011/12/22/quantropy/}
...
\subsubsection{Differential Entropy}\label{sec:differential_entropy}
...
\subsubsection{Bandwidth}\label{sec:bandwidth}
...
\fist Quantitative Feedback Theory (\S\ref{sec:quantitative_feedback})
...
is an example of a Gaussian Process (\S\ref{sec:gaussian_process})
...
the Decoder of a Transformer Model (\S\ref{sec:transformer_model}) is
Auto-regressive at Inference Time
