...
multiple Coordinates $[x, y, z]$ are associated with a single Point $(x, y)$:
$[x, y, 1]$ and any Scalar Multiple $[zx, zy, z]$

$[x, y]$ -- Projective Line; a Point $(x)$ is represented as $[x, 1]$, or any
Scalar Multiple $[yx, y]$
...
% ==============================================================================
\section{Grassmannian Space}\label{sec:grassmannian_space}
% ==============================================================================
...
% ------------------------------------------------------------------------------
\subsection{Projective Range}\label{sec:projective_range}
% ------------------------------------------------------------------------------
...
a \emph{Projective Linear Transformation} (\emph{Homography}, \emph{Projective
Collineation}, \emph{Projectivity}) is an Isomorphism of Projective Spaces
...
a composition of two or more Perspectivities (\S\ref{sec:perspectivity})

\emph{Fundamental Theorem of Projective Geometry}: if the Geometric Dimension of
a (Pappian) Projective Space is at least 2, then every Collineation is the
Product of a Homography and an Automorphic Collineation
(\S\ref{sec:automorphic_collineation})

viewing an Affine Space as the Complement of a Hyperplane at Infinity of a
Projective Space, Affine Transformations (\S\ref{sec:affine_transformation}) are
Projective Transformations of the Projective Space that leave the Hyperplane at
Infinity Invariant, restricted to the Complement of that Hyperplane



\subsubsection{Perspectivity}\label{sec:perspectivity}



% ------------------------------------------------------------------------------
\subsection{Projectivization}\label{sec:projectivization}
% ------------------------------------------------------------------------------

associates to a Non-zero $n$-dimensional Vector Space $V$ an $n-1$-dimensional
Projective Space $P(V)$ of the One-dimensional Subspaces of $V$

any Subset $S \subseteq V$ closed under Scalar Multiplication defines a Subset
of $P(V)$ called the \emph{Projectivization} of $S$

\url{https://johncarlosbaez.wordpress.com/2018/12/27/geometric-quantization-part-3/}:

Quantization (\S\ref{sec:geometric_quantization}) and Projectivization are
Adjoint Functors



% ------------------------------------------------------------------------------
\subsection{Projective Completion}\label{sec:projective_completion}
% ------------------------------------------------------------------------------

Embedding of an $n$-dimensional Vector Space $V$ over Field $K$ into the
$n$-dimensional Projective Space $P(V \oplus K)$, associating to every $v \in V$
to the Line Spanned by the Vector $(v, 1) \in V \oplus K$
...
the \emph{Real Projective Space}, $\proj_n(\reals)$ (or $\mathsf{RP}^n$) is the
Compact (\S\ref{sec:compact_space}) $n$-dimensional Smooth Manifold
(\S\ref{sec:smooth_manifold}) of \emph{Lines} passing through the Origin in
$\reals^{n+1}$, formed by taking the Quotient (\S\ref{sec:quotient_space}) of
$\reals^{n+1} \ \{0\}$ under the Equivalence Relation $x \sim \lambda x$ for all
$\lambda \neq 0$

since there is always a $\lambda$ such that $\lambda x$ has Norm one and there
are precisely two such $\lambda$ (differing by sign), $\proj_n(\reals)$ can also
be formed by identifying Antipodal Points of a Unit $n$-sphere, $S^n$, in
$\reals^{n+1}$, and can be further restricted to the upper Hemisphere of $S^n$
and only identify Antipodal Points on the Bounding Equator, which shows that
$\proj_n(\reals)$ is also equivalent to the closed $n$-dimensional Disk, $D^n$,
with Antipodal Points on the Boundary, $\partial D^n = S^{n-1}$ identified

a special case $\mathsf{Gr}(1, \reals^{n+1})$ of a Grassmannian Space
(\S\ref{sec:grassmannian_space})



\subsubsection{Real Projective Line}\label{sec:real_projective_line}
...
\label{sec:quaternionic_projective_space}
% ------------------------------------------------------------------------------
...
$k$-dimensional Linear Subspaces (\S\ref{sec:linear_subspace}) of $V$, e.g. the
Grassmanian $Gr(1,V)$ is the Space of Lines through the Origin in $V$ (and
...
over the Real Numbers, an Affine Subspace is called a \emph{Linear Manifold}
...
viewing an Affine Space as the Complement of a Hyperplane at Infinity of a
Projective Space, Affine Transformations are Projective Transformations of the
Projective Space that leave the Hyperplane at Infinity Invariant, restricted to
the Complement of that Hyperplane

\fist Projective Collination (Homography \S\ref{sec:projective_transformation})
...
Affine Group $Aff(n,F)$ -- Extension of the General
Linear Group $GL(n,F)$ by Group of Translations in $F^n$ is the Group of all
...
and Translation; in Self-similarity the object is scaled equally in all
directions, in Self-affinity the object may be scaled differently in different
directions
...
cf. Self-affinity (\S\ref{sec:self_affinity}) -- in Self-similarity the object
is scaled equally in all directions, in Self-affinity the object may be scaled
...
if the Input to an Linear Time-Invariant (LTI \S\ref{sec:lti}) System is a
Sinusoid, then the Output will also be a Sinusoid
...
cf. Affine Differential Geometry (\S\ref{sec:affine_differential}) -- Volume
Forms over a Manifold instead of Metrics
...
\subsubsection{Harmonic Map}\label{sec:harmonic_map}



% ------------------------------------------------------------------------------
\subsection{Discrete Differential Geometry}
\label{sec:discrete_differential_geometry}
% ------------------------------------------------------------------------------

\url{http://geometry.cs.cmu.edu/ddg},
\url{https://www.youtube.com/playlist?list=PL9_jI1bdZmz0hIrNCMQW1YmZysAiIYSSS}



% ------------------------------------------------------------------------------
\subsection{Affine Differential Geometry}\label{sec:affine_differential}
% ------------------------------------------------------------------------------

Volume Forms (\S\ref{sec:volume_form}) over a Manifold instead of Metrics
(Riemannian Differential Geometry \S\ref{sec:riemannian_geometry})
...
Information Geometry (\S\ref{sec:information_geometry})

\url{https://johncarlosbaez.wordpress.com/2021/08/05/information-geometry-part-18/}
-- Momentum as Surprisal (\S\ref{sec:information_content})
...
Position, Extensive Variables, Probabilities

Momentum, Intensive Variables, Surprisals

Action, Entropy, Shannon Entropy

in Classical Mechanics, Action is Minimized, in Thermodynamics, Entropy is
Maximized, and in the ``Maximum Entropy'' approach to Probability, Shannon
Entropy is Maximized



\subsubsection{Geometric Quantization}\label{sec:geometric_quantization}

\url{https://johncarlosbaez.wordpress.com/2018/12/27/geometric-quantization-part-3/}:

Quantization and Projectivization (\S\ref{sec:projectivization}) are Adjoint
Functors
...
\url{https://golem.ph.utexas.edu/category/2024/08/dimensional_analysis_in_algebr.html}
...
(wiki): determines in a systematic manner the \emph{Characteristic Units} of a
System to ``use''



\subsubsection{Characteristic Unit}\label{sec:characteristic_unit}

\begin{itemize}
  \item Time Constant $\tau$ -- main Characteristic Unit of a Linear
    Time-Invariant System (\S\ref{sec:lti_system})
  \item ...
\end{itemize}
