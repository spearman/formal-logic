...
% ------------------------------------------------------------------------------
\subsection{Pure Strategy}\label{sec:pure_strategy}
% ------------------------------------------------------------------------------

% ------------------------------------------------------------------------------
\subsection{Mixed Strategy}\label{sec:mixed_strategy}
% ------------------------------------------------------------------------------
...
\url{https://www.argmin.net/p/patterns-predictions-and-actions-1eb}

\emph{Metrical Determinism}: declaring how to evaluate Predictions determines
how to make the Predictions

given a Metric, one can determine the \emph{Optimal} Decision Rule
(\S\ref{sec:decision_rule})

Decision Theory says what the Optimal Predictions would be if all Data is known
in advance

\fist cf. Supervised Learning (\S\ref{sec:supervised_learning}) -- find Optimal
Predictions from a subset of the Data
...
\emph{Metrical Determinism}: given a Metric, one can determine the
\emph{Optimal} Decision Rule
...
\subsubsection{Individual Risk}\label{sec:individual_risk}

2014 - Dawid - \emph{On Individual Risk}

\fist Outcome Indistinguishability (\S\ref{sec:outcome_indistinguishability})
...
gap between a Policy and its Counterfactual Outcome
...
Adaptive Experimentation: Reinforcement Learning
(\S\ref{sec:reinforcement_learning})
...
cf. Sensitivity Function (\S\ref{sec:sensitivity_function})
...
Adaptive Experimentation (Exploration \S\ref{sec:exploration})
...
\subsubsection{Multi-armed Bandit}\label{sec:multiarmed_bandit}

\subsubsection{Deep Reinforcement Learning}\label{sec:deep_rl}

\fist Approximate Dynamic Programming
(\S\ref{sec:approximate_dynamic_programming})
