...
(\S\ref{sec:arithmetic_algebraic_geometry}): study of Zero Sets
(\S\ref{sec:zero_set}) of Systems of Polynomial Equations
(\S\ref{sec:system_of_polynomial_equations}) over the Rationals
...
\emph{Archimedean Property} (\S\ref{sec:archimedian_property}): the Reals are
an Archimedian Field (\S\ref{sec:archimedean_field}), i.e. for any Real Number,
...
\emph{Polar Form}: Polar Coordinates (\S\ref{sec:polar_coordinate})

Argument (Angle) $\theta$

Modulus (Radius) $r$

$r(cos\theta + i sin\thetar) = e^{i\theta}$
...
in Polar Coordinates (\S\ref{sec:polar_coordinate}): Magnitude or
...
(\S\ref{sec:transcendental_number}).

\fist cf. Arithmetic Algebraic Geometry
(\S\ref{sec:arithmetic_algebraic_geometry})
