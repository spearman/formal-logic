...
For any $p(x) \in \mathbb{Z}[x]$ and $a \equiv b \mathrm{mod}\;n$,
...
\[
  (x+y)^p \mathrm{mod}\;n \equiv x^p + y^p \mathrm{mod}\;n
\]
...
$a \equiv b (\mathrm{mod}\;n)$ \emph{Congruent modulo $n$}

For two Integers $a,b \in \mathbb{Z}$, $a$ and $b$ are in the same
Quotient Set or \emph{Residue} (\S\ref{sec:residue}) if and only if
$(a - b)|n$, denoted $a \equiv b (\mathrm{mod}\;n)$.

\begin{enumerate}

  \item $(\forall a \in \mathbb{Z}) a \equiv a (\mathrm{mod}\;n)$

  \item $(\forall a,b \in \mathbb{Z}) a \equiv b (\mathrm{mod}\;n)
    \Leftrightarrow b \equiv a (\mathrm{mod}\;n)$

  \item $(\forall a,b,c \in \mathbb{Z}) a \equiv b (\mathrm{mod}\;n)
    \wedge b \equiv c (\mathrm{mod}\;n) \Rightarrow a \equiv c
    (\mathrm{mod}\;n)$

  \item
    If $a \equiv b (\mathrm{mod}\;n)$ and $c \equiv d (\mathrm{mod }
    n)$, then:
    \begin{itemize}
    \item $a + c \equiv b + d (\mathrm{mod}\;n)$
    \item $a - c \equiv b - d (\mathrm{mod}\;n)$
    \item $ac \equiv bd (\mathrm{mod}\;n)$
    \item $a^t \equiv b ^t (\mathrm{mod}\;n)$ for any $t \in
      \mathbb{N}$
    \end{itemize}

\end{enumerate}

Cancellation: If $ac \equiv bc (\mathrm{mod}\;n)$ then $a \equiv b
...
$\mathrm{mod}\;n$ if $|T| = n$ and no Elements of a particular Residue
$\mathrm{mod}\;n$ occur more than once.

For a CSR $\mathrm{mod}\;n$ $T = \{a_1, a_2, \ldots, a_n\}$ and an
Integer $c \in \mathbb{Z}$ Coprime with $n$ ($gcd(c,n) = 1$), for any
Integer $b \in \mathbb{Z}$, the Set $S = \{b + ca_1, b + ca_2, \ldots,
b + ca_n \}$ is also a CSR $\mathrm{mod}\;n$.
