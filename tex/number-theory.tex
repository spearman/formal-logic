...
Ordered Pair $(a,b)$ where $a,b \in \reals$

$a + bi$

$\comps = \{ (a,b) | a,b \in \reals \}$

For $a \in \reals$, $(a,0)$ is Identified with the Real Number $a$

Complex Plane (\S\ref{sec:complex_plane})

Polar Coordinates (\S\ref{sec:polar_coordinate})

Algebraically Closed Field (\S\ref{sec:closed_field} -- Fundamental
Theorem of Algebra: extending the Reals with the solution to the
equation $i^2 + 1 = 0$ is Algebraically Closed
...
\subsubsection{Period Number}\label{sec:period_number}
