...
\emph{Training Set} (\S\ref{sec:training_set})
...
% ------------------------------------------------------------------------------
\subsection{Training Set}\label{sec:training_set}
% ------------------------------------------------------------------------------

cf. Statistical Sample (\S\ref{sec:sample}), Dataset (\S\ref{sec:dataset})

\fist Learning Process (\S\ref{sec:learning_process})
...
(wiki): any Multi-Layer Peceptron (MLP \S\ref{sec:mlp}) Network using a Linear
...
this ``Layer'' is \emph{not counted}, e.g. a Single-layer Perceptron has Input
Variables and an Output Layer only, an MLP with a single Hidden Layer is a
2-layer MLP
...
Training Set (\S\ref{sec:training_set})
...
cf. Gauss-Newton Algorithm (Non-linear Least Squares \S\ref{sec:nlls})
...
(\S\ref{sec:sgd}) uses the Gradients for ``Training'' the Model via Optimization
