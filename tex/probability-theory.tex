...
a Random Variable defined for any Event regardless of whether a Random Variable
is being measured or not. (wiki)
...
Variable or Signal (\S\ref{sec:signal}) is the amount of Information
(\S\ref{sec:information}) ``gained'' by the Sample; this Information Content is
a Random Variable defined for any Event (\S\ref{sec:probability_event})
...
Marginal Distribution of $Y$ is $Y \sim Poisson(\lambda p)$ (Wasserman04 \S 23.3)
...
for any Event (\S\ref{sec:probability_event}) regardless of whether a Random
...
to measure Fit in Logistic Regression (\S\ref{sec:logistic_regression})
...
\begin{itemize}
  ...
  \item in the general case $\vec{y} = \mathbf{X}\vec{\beta}$, solving the
    ...
    \]
    note that this solution is \emph{Ill-conditioned}
...
\paragraph{Logistic Regression}\label{sec:logistic_regression}
...
maximizing $\mathcal{L}(B)$ numerically using Re-weighted Least Squares
