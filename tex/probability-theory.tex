...
\textbf{Thm.} (Weak Law of Large Numbers) \emph{If $X_1, \ldots, X_n$ are IID
  (\S\ref{sec:iid}), then:
  \[
    \overlina{X}_n \xrightarrow{P} \mu = E(X_1)
  \]
}
...
Statistical Model $(S,\mathcal{P})$ is a Sample Space (\S\ref{sec:sample_space})
...
\emph{Regression Variable}, ``Regressor'', or ``Explanatory Variable'' --
Independent Variable (\S\ref{sec:independent_variable}); Covariate

\emph{Response Variable}, ``Regressand'', or ``Explained Variable'' -- Dependent
...
of the Dependent Variable around the Prediction (\S\ref{sec:prediction}) of the
Regression Function using a Probability Distribution
...
Estimate (\S\ref{sec:estimation_theory}) of the Interval in which future
Observations (\S\ref{sec:observation}) will fall with a certain Probability,
