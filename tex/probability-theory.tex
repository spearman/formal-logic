...
for each fixed $V$, the Parameter $(\Theta, *)$ is a Contravaraint Functor
$\cat{O} \to \cat{K}$, where $\cat{K}$ is usually the Category of Surjective
Linear Transformations on Vector Spaces, associating each Covariate Set $\Omega$
with a Parameter Set $\Theta_\Omega$, and each Injective Covariate Morphism
$\varphi_c : \Omega \to \Omega'$ with a Surjective Parameter map
$\varphi_c^* : \Theta_{\Omega'} \to \Theta_\Omega$
