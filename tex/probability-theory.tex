...
a Probability Distribution is given by its Continuous Distribution Function
...
$x_0 = 0$ \emph{does not} Converge to $0$ as $n \to \infty$ with
...
$X_t - X_s$ or Subset of a Multi-dimensional Space
...
$(V, *)$ is a \emph{Functor} on $\cat{U}$ associating each $U$ with the Sample
Space $V^U$ and with each Morphism $\varphi : U \to U'$ the map
...
\emph{Sample Space Category} $\cat{S}$ -- Objects $V^U$ (with $V \in \cat{V}$ and
$U \in \cat{U}$) and Morphisms $V^{U'} \to V'^{U}$ are Compositions
$\gamma_U(\varphi^*_V) = \varphi^*_{V'}(\gamma_{U'})$ where $\gamma : V \to V'$
...
a Subrepresentation $\Theta$ in the standard Representation $V^{\Omega}$ is a
...
in the usual complete formulation, the Parameter Space is extended to $\Theta
\times \reals^+$ by the inclusion of a Dispersion Parameter $\sigma^2$ and for
the Design $\psi : U \to \Omega$, the Sample Space is $V^U$, and the full
Normal-theory Linear Model is the Set of Normal Distributions with Mean Vector
in the Subspace $\psi^*\Theta_\Omega$, and the Variance Matrix is ``Proportional
to the Identity'' (FIXME: clarify)
...
a \emph{Statistical Model} is then a Functor on the Design Category $\cat{D}$
associating with each Design Object $\psi : U \to \Omega$ a \emph{Model Object}
that is a map $P_\psi : \Theta_\Omega \to \mathcal{P}(S)$ such that
$P_\psi\theta$ is a Probability Distribution on $S$, and to each Morphism
$(\varphi_d,\varphi_c) : \psi \to \psi'$ in $\cat{D}$ is associated a map
$(\varphi^\dagger_d, \varphi^*_c) : P_{\psi'} \to P_\psi$ (FIXME: what category
is this Functor *into* ???); the Set of Distributions generated is
$\mathcal{F}_\psi = P_\psi \Theta_\Omega$

as a result of this definition:
\begin{enumerate}
  \item the Sample-space Transformation
    $\varphi^*_d : S' \to S$ also carries each Distribution $F$ on $S'$ to a
    transformed Distribution $\varphi^\dagger_d F = F \circ \varphi^{*-1}_d$
    on $S$,
  \item by Commutativity,
    \[
      P_\psi\varphi^*_c =
        \varphi^\dagger_d P_{\psi'} : \Theta_{\Omega'} \to \mathcal{P}(S)
    \]
    that is,
    \[
      \theta \in \Theta_{\Omega'}, P_\psi \varphi_c^* \theta =
        \varphi^\dagger_d P_{\psi'} \theta
    \]
\end{enumerate}
(1) ensures that the Family of Distributions on $S$ is ``\emph{suitably
embedded}'' in the Family of Distributions on $S'$-- the maps
$\varphi^*_d : S' \to S$ that define the embedding depend on the Category of
...
``consistency condition'' (2) asserts taht the Distribution $P_\psi\theta$ on
$S$ is \emph{the same} as the Marginal Distribution of $P_{\psi'}\theta$ on $S'$
by the embedding map $\varphi_d^* : S' \to S$
...
Daniell-Kolmogorov Extension Theorem -- a suitably ``consistent'' collection of
Finite-dimensional Distributions will define a Stochastic Process
