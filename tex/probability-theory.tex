...
\begin{itemize}
  \item $z$-score (Standard Score or $z$-statistic \S\ref{sec:standard_score})
  ...
  \item $t$-statistic (\S\ref{sec:t_statistic}) -- Estimation of Population Mean
    ...
    Means of the Population Standard Deviation is unknown
\end{itemize}
...
cf. $z$-score (Standard Score \S\ref{sec:standard_score})
...
\subparagraph{Midrange}\label{sec:midrange}\hfill

Average of Max and Min



\paragraph{InterQuartile Range (IQR)}\label{sec:iqr}\hfill
...
cf. Standard Error (\S\ref{sec:standard_error})

\fist Statistics (\S\ref{sec:statistic}) --
Estimation of Standard Deviation of Population Proportion
(\S\ref{sec:population_proportion}) from the Sample mean and Sample Standard
Deviation

cf. $t$-statistic (\S\ref{sec:t_statistic}) for Estimating Population Mean
...
cf. Regression Error, Mean Square Deviation (MSD \S\ref{sec:regression_error})
...
cf. Mean Squared Deviation (Mean Square Error \S\ref{sec:mse})
...
cf. Residual (\S\ref{sec:residual})
...
cf. Mean Absolute Deviation (MAD)
...
Value and the \emph{Estimated} (\S\ref{sec:estimation_theory}, cf. Predicted,
Expected) Value
...
cf. Parameter Estimation (\S\ref{sec:estimation_theory})
...
finding a Classifier is based on a \emph{Training Dataset} (\S\ref{sec:dataset})
...
cf. Neural Networks (\S\ref{sec:ann})
...
Linear Model:
\[
  \sum_n (y_i - \beta_1 x_i + \beta_0)
\]
can be re-written as:
\[
  n (\overline{y^2} - 2 \beta_1 \overline{x y} - 2 \beta_0 \overline{y}
    + \beta_1^2 \overline{x^2} + 2 \beta_1 \beta_0 \overline{x} + \beta_0^2)
\]
finding where the Partial Derivative with respect to $\beta_0$ and $\beta_1$ are
both Zero gives the Least-squares Estimate (\S\ref{sec:least_squares}) of the
Regression Function
...
\]
and substituting to solve for $\beta_0$:
\[
  \beta_0 = \overline{y} - \frac{
    \overline{x} Cov(x,y)
  }{
    Var(x)
  }
...
<https://www.youtube.com/watch?v=FGesqq22TCM>: Equation for a Regression Line
passing through the Sample Mean $(\overline{X}\overline{Y})$ with Slope
...
special case of General Linear (Multivariate) Regression
(\S\ref{sec:multivariate_regression})
...
\fist a Logistic Regression Model (\S\ref{sec:logistic_regression}) can be
formulated as a specific case of Generalized Linear Model



\subparagraph{Link Function}\label{sec:link_function}\hfill
...
Regression as a Discriminative (\S\ref{sec:discriminative_model}) methods of
...
of Stochastic Process where the Index Set is some Subset of the Integers
...
\]
is called a \emph{Transition Probability}, and the Matrix $\mathbf{P}$ of
Transition Probabilities is called the \emph{Transition Matrix};
$p_{ij} \geq 0$, and each Row is a Probability Mass Function, since
$\sum_i p_{ij} = 1$

a Markov Process is \emph{Homogeneous} if Transition Probabilities $P(X_{n+1} =
j | X_n = i)$ is Constant over Time, i.e.:
\[
