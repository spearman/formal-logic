...
(\S\ref{sec:scaling_distribution}) $U$ is obtained by $U = e^V$ (Mandelbrot97)
...
\subparagraph{Covariance Function}\label{sec:covariance_function}\hfill
...
Covariance of a Random Variable Process (\S\ref{sec:stochastic_process}) or
Field (\S\ref{sec:random_field}) $Z(x)$ on a Domain $D$ at two
``\emph{locations}'' $x$ and $y$:
\[
...
\fist Cross-covariance (\S\ref{sec:cross_covariance}) -- Covariance between two
...
\begin{itemize}
  \item Explicitly
    \begin{itemize}
      ...
      \item Approximate Density -- Variational Auto-encoders
        (\S\ref{sec:variational_autoencoder})
    \end{itemize}
  \item Implicitly
    \begin{itemize}
...
particular type of Tikhonov Regularization (\S\ref{sec:tikhonov_regularization})
useful to mitigate Multicollinearity (\S\ref{sec:multicollinearity})
...
% ------------------------------------------------------------------------------
\subsection{Autocovariance}\label{sec:autocovariance}
% ------------------------------------------------------------------------------

cf. Autocorrelation (\S\ref{sec:autocorrelation})
%FIXME: move section?

\fist Covariance Function (\S\ref{sec:covariance_function})



% ------------------------------------------------------------------------------
\subsection{Cross-covariance}\label{sec:cross_covariance}
% ------------------------------------------------------------------------------
...
\subparagraph{Cauchy Process}\label{sec:cauchy_process}\hfill
