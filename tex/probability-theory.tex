...
Normal Equations:
\[
  (\mathbf{J} \mathbf{W J}) \mathbf{Delta} \vec{\beta} =
    \mathbf{J} \mathbf{W \Delta} \vec{y}
\]

\emph{Gauss-Newton Algorithm}
...
\fist Neural Networks (\S\ref{sec:ann}) -- a Perceptron is an \emph{Artificial
  Neuron} (\S\ref{sec:artificial_neuron}) using the Unit Step Function
(\S\ref{sec:unit_step_function}) as the Activation Function

\fist Single-layer Perceptron (\S\ref{sec:single_layer_preceptron})
...
Feed-Forward Neural Network (FFNN \S\ref{sec:ffnn})
...
\begin{itemize}
  \item Perceptron (\S\ref{sec:perceptron}) -- Neuron using the Unit Step
    Function (\S\ref{sec:unit_step_function}) as the Activation Function
  \item ...
\end{itemize}
...
sometimes ``Transfer Function'', not to be confused with Transfer Function
(Evolution Function \S\ref{sec:evolution_function}) of a Dynamical System
...
cf. Perceptron (\S\ref{sec:perceptron}) -- Linear Classifier



\paragraph{Multi-Layer Perceptron (MLP)}\label{sec:mlp}\hfill

\paragraph{Convolutional Neural Network}\label{sec:cnn}\hfill
