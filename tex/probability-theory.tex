...
Parametric Models (\S\ref{sec:parametric_model}) have a Finite Set
of Parameters
...
for Bernoulli and Normal Distributions, Posterior Mean
(\S\ref{sec:posterior_distribution}) is generally close to the MLE
...
(\S\ref{sec:null_hypothesis}), $H_0 : \theta \in \Theta_0$, represents any
Hypothesis, and the \emph{Alternative Hypothesis}
(\S\ref{sec:alternative_hypothesis}), $H_1 : \theta \in \Theta_1$, represents an
Hypothesis that is accepted in the case that the Null Hypothesis is Rejected
...
\[
  H_0 : \theta \leq \theta_0 \quad\quad\quad H_1 : \theta > \theta_0
\]
or:
\[
  H_0 : \theta \geq \theta_0 \quad\quad\quad H_1 : \theta < \theta_0
\]

\emph{Two-tail (Two-sided) Test}:
\[
  H_0 : \theta = \theta_0 \quad\quad\quad H_1 : \theta \neq \theta_0
\]
...
$H_0 : \theta = \theta_0$ and Simple Alternative Hypothesis
$H_1 : \theta = \theta_1$
...
A \emph{Statistical Hypothesis} is a Conjecture (\S\ref{sec:conjecture})
concerning one or more Populations (\S\ref{sec:population}).
...
\textbf{Thm.} \emph{
  ...
  $H_0 : \theta = \theta_0$ the $p$-value has a $Uniform(0,1)$ Distribution and
  therefore one Rejects $H_0$ when $p$-value is less than the Significance Level
  $\alpha$, and the Probability of a Type I Error is $\alpha$.
}
...
used for comparing Fit (\S\ref{sec:fit}) of two Statistical Models
...
for Bernoulli and Normal Distributions, Posterior Mean is generally close to the
MLE (Mean Likelihood Estimate \S\ref{sec:mle})
...
Maximum Likelihood Estimation (MLE \S\ref{sec:mle}) -- $\hat{\theta}$: the value
of $\theta$ that Maximizes $\mathcal{L}(\theta)$
