...
\emph{independent} of $q$

--FIXME: $\alpha$ ???

\emph{Multiscaling} (Multifractal) -- the Exponents of the Scale Factors
\emph{depend} on $q$
...
Subordinated (\S\ref{sec:subordination}) to a Process associated with a L\'evy
Distribution
...
cf. Fractal Dimension (\S\ref{sec:fractal_dimension}) -- Hurst Exponent
...
Mild (Gaussian) -- Short-run and Long-range even-ness, Tail-mixing

Slow (Lognormal) -- Short-run Concentration and Long-range even-ness,
Tail-preserving

Wild (Scaling) -- Short-run and Long-run Concentration, Tail-preserving

Long-run ($N \to \infty$) -- Pre-Guassian (``even'' in the Long-run): $1 / N \to
0$; ``Theory of Errors'', ``absence of Concentration in the Long-run'' (cf.
Central Limit Theorem \S\ref{sec:central_limit})-- the assumption that Errors
are Gaussian because they are the Observable Outcome of a large number of
separate Additive contributions such that even the largest is negligible
compared to the Sum both \emph{ex-ante} (in terms of Distributions) and
\emph{ex-post} (in terms of Realized values)
...
\[
  A_N \sum_{n=1}^N U_n - N \expect{U}
\]
where $A_N \sim \frac{1}{\sqrt{N}}$ (FIXME: distributed as a distribution or is
this a typo ???), Limit is a Gaussian as $N \to \infty$; the Domain of
``Universality of Attraction'' to the Gaussian includes all $U$ satisfying
$\expect(U^2) < \infty$ and also some cases where $\expect(U^2)$ Diverges
``slowly enough''
...
if $U_n$ is in the Domain of Universality of $\alpha < 2$, the Limit is a Random
Variable called \emph{$L$-stable} (\S\ref{sec:lsm})

neglecting the slowly varying term $L(u)$:
\[
  N^{\frac{1}{\alpha}} \sum_{n=1}^N U_n - B_N
\]
where $B_N$ is $N \expect(U)$ when $1 < \alpha < 2$ and $0$ when
$0 < \alpha < 1$; the Sum $U_N$ is Asymptotically of order
$N^{\frac{1}{\alpha}}$
(FIXME: explain)
...
Doubling Convolution, Portioning Ratio
...
``Extreme'' Randomness -- Log-cauchy (\S\ref{sec:log_cauchy})
...
cf. Fractal Time (\S\ref{sec:fractal_time})
...
can be seen as a Brownian Motion Subordinated (\S\ref{sec:subordination}) to a
