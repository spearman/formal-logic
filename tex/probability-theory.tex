...
\begin{enumerate}
  ...
  \item ``Concentration in Probability'' -- involves a Limit Theorem
\end{enumerate}
...
level'' of Statistical Dependence; cf. properties of a ``gas'' as a state of
matter
...
\]
where $A_N \sim \frac{1}{\sqrt{N}}$, Limit is a Gaussian as $N \to \infty$; the
Domain of ``Universality of Attraction'' to the Gaussian includes all $U$
satisfying $\expect(U^2) < \infty$ and also some cases where $\expect(U^2)$
