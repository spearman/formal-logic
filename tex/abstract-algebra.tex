...
String Rewriting (\S\ref{sec:string_rewriting}) %FIXME: ???
...
Product (\S\ref{sec:cartesian_product}) of the Sets with the Operations defined
coordinatewise.

Direct Product on Sets coincides with the usual notion of Cartesian Product.
...
\]
where $x$ is called a \emph{Pseudoinverse}. Every Group (\S\ref{sec:group}) is a
Regular Semigroup.
...
\subsubsection{Infinite Cyclic Group}\label{sec:infinite_cyclic}
...
\subparagraph{Tensor Representation}\label{sec:tensor_representation}\hfill
...
\subparagraph{Equi-affine Group}\label{sec:equiaffine_group}\hfill
...
in every Group $G$, Left Group Multiplication is an Action of $G$ on itself:
...
\subsubsection{Locally Compact Group}\label{sec:locally_compact_group}

\paragraph{Amenable Group}\label{sec:amenable_group}\hfill
...
\subsubsection{Prime Element}\label{sec:prime_element}
...
``\emph{Given a (Finite, Galois) Field Extension $E/F$, there is a one-to-one
