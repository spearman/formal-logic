...
\emph{Hereditarily Finite Sets} (Sets that have Finite Pictures).
\cite{aczel88}
%FIXME ref hereditarily finite
...
\paragraph{Transitive Reduction}\label{sec:transitive_reduction}
...
AFA (Aczel) -- every Accessible Pointed Graph
(\S\ref{sec:pointed_graph}) corresponds to (the Membership Structure
of) a Unique Set (every Graph has a Unique Decoration); Maximal
Bisimulation Relation %FIXME

SAFA (Scott) -- Sets are Equal if Isomorphic

FAFA (Finsler) -- Sets are Equal if Isomorphic (different sense)

BAFA (Boffa) -- standard Extensionality as Equality: Sets are Equal if
...
Isomorphism Relations: SAFA, FAFA

Extensionality: BAFA

Decoration, Unfolding %FIXME

Finite Pictures: Hereditarily Finite Sets, e.g. Quine Atom
(\S\ref{sec:quine_atom})

\textbf{Thm.} Any Final Coalgebra (\S\ref{sec:terminal_coalgebra}) for
$\pow$ is a Model for the Anti-foundation Axiom \cite{aczel88}



\paragraph{Labelled Anti-foundation Axiom}
\label{sec:labelled_antifoundation}
\hfill \\
...
\subsubsection{Quine Atom}\label{sec:quine_atom}
...
\paragraph{Hyperset}\label{sec:hyperset}
