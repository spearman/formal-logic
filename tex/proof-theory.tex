...
(\emph{WFFs}) or just \emph{Formulas}.

A Formula is \emph{Satisfiable} (\S\ref{sec:satisfaction}) if it is
True under some \emph{Interpretation} (\S\ref{sec:interpretation}).
...
\emph{Well-formed Formulas} \S\ref{sec:formal_system}), $\mathcal{E}$,
consisting of \emph{Elementary Theorems} considered to be \emph{True}.
In a \emph{Deductive Theory} (\S\ref{sec:deductive_theory}),
$\mathcal{T}$ is an \emph{Inductive Class} such that some of the
Elementary Theorems are taken to be \emph{Axioms} and any Expressions
Deducible by those Axioms are also Elementary Theorems in
$\mathcal{T}$.
...
Systems of Natural Deduction, contrasted with Axiomatic Systems
(\S\ref{sec:axiomatic_system}), include many Inference Rules but few or
