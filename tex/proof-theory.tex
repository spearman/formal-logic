...
\emph{Proof Theory} studies \emph{Deductive Inference} through
\emph{Formal Proofs} (\S\ref{subsec:formal_proof}).

Proof by \emph{Syntactic Consequence} (a form of \emph{Logical
  Consequence} (\S\ref{sec:logical_consequence})) is a \emph{Deductive
  Argument}; that is, the production of a \emph{Theorem}
(\S\ref{subsec:theorem}) is said to be a \emph{Derivation}. Deduction
proper is the top-down, \emph{Reductive} process that starts with
general Axioms and Reduces to the specific Theorem
(\S\ref{subsec:formal_proof}) that is being proved. \emph{Mathematical
  Induction} is the bottom-up, \emph{Implicative} process where a
\emph{Base Case} is shown to extend to the more general by means of
Implication (the Inductive step). Note that Mathematical Induction is
not \emph{Inductive Reasoning} which is an empirical or probabilistic
Inference and not a form of Deduction. The \emph{Principle of
  Mathematical Induction} (\S\ref{subsec:induction_principle}) is an
application of Mathematical Induction to the \emph{Natural Numbers}
...
A \emph{Formal System} (or \emph{Logical Calculus}) is a Formal
Language combined with a \emph{Deductive Apparatus}
(\S\ref{subsec:deductive_apparatus}) in order to \emph{Derive}
\emph{Theorems} (\S\ref{subsec:formal_proof}). Here, Expressions
(strings) of Symbols are usually referred to as \emph{Forumulas}, and
Expressions that belong to the Language are called ``\emph{Well-formed
  Formulas}'' (\emph{WFF}).
...
that \emph{Falsity} is not provable starting with zero
\emph{Assumptions}.

Symbols may be divided into \emph{Logical} and \emph{Non-Logical
  Symbols}: Logical Symbols (e.g. $\forall$, $\vee$, $\rightarrow$,
$\neg$, etc., and \emph{Variables} $x_0$, $x_1$, etc.) always have the
same meaning while Non-Logical Symbols (e.g. \emph{Predicates}
(Relations), \emph{Functions}, and individual \emph{Constants})
(\S\ref{sec:logic_terminology}) only have meaning under
\emph{Interpretation} (\S\ref{subsec:interpretation}). The set of
...
zero or more \emph{Axioms} and one or more \emph{Inference Rules}. A
Deductive System gives a precise account of \emph{Logical Consequence}
with respect to a Formal Language. A Formal System is termed
...
% --------------------------------------------------------------------
\subsection{Axioms}\label{subsec:axiomatization}
% --------------------------------------------------------------------
...
% --------------------------------------------------------------------
\subsection{Inference Rules} \label{subsec:inference_rules}
% --------------------------------------------------------------------
