...
\emph{Proof Theory} is the study of \emph{Deductive Logic}
(\emph{Top-down Logic} or \emph{Deductive Reasoning}) by means of
representing \emph{Deductive Arguments} as \emph{Formal Proofs}
(\S\ref{sec:formal_proof}).

Formal Proofs proceed by \emph{Syntactic Consequence}
(\S\ref{sec:syntactic_consequence}) to make \emph{Deductive Inferences}
(\S\ref{sec:deductive_inference}).
...
A \emph{Formal System} (also \emph{Logical Calculus} or \emph{System
  of Inference}) is a Formal Language combined with a \emph{Deductive
  Apparatus} (\S\ref{sec:deductive_apparatus}) in order to
\emph{Derive} \emph{Theorems} (\S\ref{sec:formal_proof}). Here,
Expressions (strings) of Symbols are usually referred to as
...
is, it allows the Derivation of only the affirmation or the denial of
a particular Fomula, not both. Another way of stating the Consistency
requirement is that \emph{Falsity} is not provable starting with zero
\emph{Assumptions} (that is, WFFs given as true without Proof).
...
zero or more \emph{Axioms} (\S\ref{sec:axiom}) and one or more
\emph{Inference Rules} (\S\ref{sec:inference_rule}). A Deductive
System gives a precise account of \emph{Logical Consequence} with
respect to a Formal Language. A Formal System is termed
...
\subsubsection{Syntactic Consequence}\label{sec:syntactic_consequence}
