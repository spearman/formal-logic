...
for the purpose of performing analysis within a Formal System. An
Axiom is properly an \emph{Inference Rule} with no Premises,
only a given \emph{Conclusion}; that is a \emph{Logical
  Assertion}(\S\ref{sec:sequent_notation}).

Axioms are special cases of Inference Rules which have no Premises,
only a universally Valid Conclusion. Axioms may be further
differentiated from Rules by saying that Rules are statements
\emph{about} the system, Axioms are statements \emph{in} the system.
