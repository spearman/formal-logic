...
%FIXME: xref utility function game theory ???
...
\subsubsection{Multiplicative Weight Update Algorithm (MWUA)}\label{sec:mwua}

\url{https://www.jeremykun.com/2017/02/27/the-reasonable-effectiveness-of-the-multiplicative-weights-update-algorithm/}
...
the Categorical Semantics of Types in a Differentiable Language are
``\emph{Additive Containers}'' (Set-indexed Families of Commutative Monoids)

the Semantics of a Type is (1) a Set of values that the Type can have in the
Forward Pass, and (2) for each of those values, a Commutative Monoid of
Derivatives it can have at that value in the Backward Pass

the Semantics of a Function $A \to B$ is an \emph{Additive Lens} between the
Additive Containers $(A,A')$ and $(B,B')$; this consists of (1) an ordinary
FUnction between Sets $f : A \to B$ describing the Semantics of the Forward Pass
and (2) an $A$-indexed Family of Monoid Homomorphisms $f'(x,-)$ from the
Commutative Monoid $B'(f(x))$ to $A'(x)$, i.e. they are Dependent Lenses whose
Backward Passes are Additive on their second input; the Chain Rule for Lens
Composition exactly describes the Denotational Semantics of how the Forwards and
Backwards Passes interact in Automatic Differntiation

a Deep Learning (\S\ref{sec:deep_learning}) architecture consists of a Lens of
the shape $f : A \times P \to B$ where $P$ is the Parameter Space
...
% ==============================================================================
\section{Offline Optimization}\label{sec:offline_optimization}
% ==============================================================================
...
% ==============================================================================
\section{Online Optimization}\label{sec:online_optimization}
% ==============================================================================

incomplete knowledge of the future

minimize a function with respect to some ``decision variable'', but the decision
variable can be changed at each iteration

problems with Online Algorithms:
\begin{itemize}
  \item Bandit Problem (\S\ref{sec:multiarmed_bandit})
  \item ... TODO: more
\end{itemize}
...
% ------------------------------------------------------------------------------
\subsection{Greedy Algorithm}\label{sec:greedy_algorithm}
% ------------------------------------------------------------------------------
...
\label{sec:approximate_dynamic_programming}

\fist Deep Reinforcement Learning (\S\ref{sec:deep_rl})
...
%FIXME: stochastic programming ?

\url{https://www.argmin.net/p/quantifying-those-unknown-unknowns}

cf. Stochastic Programming (\S\ref{sec:stochastic_programming}),
Stochastic Control (\S\ref{sec:stochastic_control})
...
Online Algorithms (\S\ref{sec:online_optimization})

Regret Minimization (\S\ref{sec:regret})

Explore-then-commit

\emph{Principle of Certainty Equivalence}; as Action speed increases, the need
for a precise Predictor decreases



\subsubsection{Contextual Bandit}\label{sec:contextual_bandit}

\url{https://www.argmin.net/p/you-take-away-the-context}

Contextual Online Optimization

Greedy Algorithm (\S\ref{sec:greedy_algorithm}) -- if Prediction Errors are
small, the greedy Action must be almost as good as the optimal Action;
\emph{Certainty Equivalence}; as Action speed increases, the need for a precise
Predictor decreases



\subsubsection{Adversarial Bandit}\label{sec:adversarial_bandit}

\url{https://banditalgs.com/2016/10/01/adversarial-bandits/}



% ------------------------------------------------------------------------------
\subsubsection{Stochastic Programming}\label{sec:stochastic_programming}
% ------------------------------------------------------------------------------

% FIXME: stochastic optimization ?

cf. Stochastic Control (\S\ref{sec:stochastic_control})
...
% ==============================================================================
\section{Policy Optimization}\label{sec:policy_optimization}
% ==============================================================================

%TODO: move section ?

\url{https://www.argmin.net/p/policy-phylogenesis} -- TODO

Reinforcement Learning (\S\ref{sec:reinforcement_learning})
