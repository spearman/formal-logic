...
(Fixed-point Logic \S\ref{sec:modal_mu_calculus})
...
fragments of Linear Logic (\emph{Subsingleton Logic} augmented with
Least Fixed-points (Baelde-Doumane-Saurin16, Fortier-Santocanale13);
Proofs allowed to be Circular with conditions to ensure they can be
seen as \emph{Coinductively Defined} (\S\ref{sec:coinduction})
...
% --------------------------------------------------------------------
\subsection{Modal $\mu$-calculus}\label{sec:modal_mu_calculus}
% --------------------------------------------------------------------
...
-- Linear Communicating Automata (\S\ref{sec:communicating_automata})
...
can be viewed as a merging of Intuitionistic Logic and Multiplicative
Intuistionistic Linear Logic

Models of Bunched-implication Proofs are given by Bicartesian Doubly
Closed Categories, i.e. Categories that combine the Semantics of
Propositional Intuitionistic Logic and Propositional Intuitionistic
...
Predicative version includes Multiplicative (or Intensional)
Quantifiers $\forall_{new}$ and $\exists_{new}$ arising from
restrictions on Structural Rules of Terms as well as Propositions
