...
\subparagraph{Basic Logic}\label{sec:basic_logic}
...
\paragraph{Vacuous Truth}\label{sec:vacuous_truth}
...
Objects. A Predicate (\S\ref{sec:predicate}) or Relation
(\S\ref{sec:relation}) is not itself the Property, but the
\emph{Extension} (\S\ref{sec:extension}) of that Property.
