...
\paragraph{Clause}\label{sec:clause}

A Finite Disjunction (\S\ref{sec:disjunction}) of Literals is called a
\emph{Clause}.

\subparagraph{Horn Clause}\label{sec:horn_clause}\hfill \\
...
A \emph{Propositional Formula} is a Formula that is assigned a Truth
Value (\S\ref{sec:truth_value}), i.e. it is a Truth-bearer
...
thus all Provable Sentences are True. The converse of Soundness,
Completeness (\S\ref{sec:completeness}) requires that all True
Sentences are Provable. By the Incompleteness Theorem
(\S\ref{sec:incompleteness_theorem}), not all Sound Deductive Systems
are Complete.
