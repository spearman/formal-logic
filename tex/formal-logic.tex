...
(\S\ref{sec:predicate}) and Function Symbols
...
(\S\ref{sec:interpretation}) as Functional Predicates
...
\[
  (F,R,ar)
\]
where $F$ is a Set of Function Symbols, and $R$ is a Set of Predicate
Symbols and $ar$ is the Arity Function:
\[
...
Predicate Symbol is a \emph{Propositional Variable}
(\S\ref{sec:propositional_logic}). A Function or Predicate Symbol $s$
...
\paragraph{Many-sorted Signature}\label{sec:many_sorted_signature}
\hfill \\
...
Individual (\S\ref{sec:individual})
...
built up from Atomic Formulas (\S\ref{sec:atomic_formula}) and Logical
Connectives (\S\ref{sec:logical_connective}).
...
Variables act as the Atomic Formulas of Propositional Logic.
...
\subsubsection{Tautology}\label{sec:tautology} \hfill \\
...
Partition (\S\ref{sec:partition}) the Domain.
...
\begin{itemize}
  ...
  \item $\mathbf{T}$, \emph{Reflexivity Axiom}:
...
Formulas under Semantic Interpretation. This usually amounts to the
simple requirement that the Axioms are Valid and the Inference Rules
preserve Validity, i.e. any Theorem Provable in the System is Valid,
...
\S\ref{sec:attribute}) are Objects that can be Predicated
(\S\ref{sec:predicate}) of other Objects. A Predicate
(\S\ref{sec:predicate}) or Relation (\S\ref{sec:relation}) is not
itself the Property, but the \emph{Extension} (\S\ref{sec:extension})
of that Property. An Extension of a Property is a Set, but not every
Property has an Extension as exemplified by Russell's Paradox
...
\paragraph{$P$-closure}\label{sec:p_closure}
