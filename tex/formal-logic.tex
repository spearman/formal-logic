...
(\S\ref{sec:predicate}) and Function Symbols (\S\ref{sec:function}).
...
Algebra (\S\ref{sec:heyting_semantics}, also a Cartesian Closed
Preorder \S\ref{sec:cartesian_preorder}) or Kripke Semantics
...
An Algebraic Semantics for Linear Logic is given by Quantales
(\S\ref{sec:quantale})
...
\subsubsection{Internal Language}\label{sec:internal_language}

Simply-typed $\lambda$ Category (Internal Language for) Cartesian
Closed Categories

Extensional Martin-L\"of Type Theories (Internal Language for)
...
which is an empirical or probabilistic Inference and not a form of
Deduction. The \emph{Principle of Mathematical Induction}
