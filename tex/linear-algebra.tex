...
\[
  \vec{x}^* A \vec{x} > 0
\]
for any $\vec{x} \neq \vec{0}$, which implies that $A$ is is a Symmetric
(\S\ref{sec:symmetric_matrix}) or Hermitian (\S\ref{sec:hermitian_matrix})
...
Variables $x_1,\ldots,x_n$ can always be written as $\vec{x}^T M \vec{x}$ for
$M$ a Symmetric Real Matrix; $M$ is a Positive-definite Matrix when $f$ has a
unique Minimum (Zero) when $\vec{x}$ is Zero and is Strictly Positive for any
other $\vec{x}$

generally a Real Function $f$ on $n$ Real Variables has local Minimum at
$\vec{x}_0$ if its Gradient (\S\ref{sec:gradient}) is Zero and its Hessian
Matrix (\S\ref{sec:hessian_matrix}) is Positive Semi-definite at that Point
(FIXME: clarify)

an \emph{Indefinite Matrix} is a Matrix that is not Positive-definite,
Positive-semidefinite, Negative-definite, nor Negative-semidefinite

\begin{itemize}
  \item $I$ -- Identity Matrix
  \item Diagonal with Positive Real Diagonal entries
\end{itemize}
...
\begin{itemize}
  \item Diagonal with Zero or Positive Real Diagonal entries
\end{itemize}
...
generally a Real Function $f$ on $n$ Real Variables has local Minimum at
$\vec{x}_0$ if its Gradient (\S\ref{sec:gradient}) is Zero and its Hessian
Matrix is Positive Semi-definite (\S\ref{sec:positive_semidefinite}) at that
Point (FIXME: clarify)
