...
Ehrhard-Regnier: (Linear) Logical (\S\ref{sec:linear_logic}) notion of
``Linear'' (using arguments exactly once) coincides with Mathematical notion of
...
\subsubsection{Affine Combination}\label{sec:affine_combination}

\paragraph{Affine Hull}\label{sec:affine_hull}\hfill

\paragraph{Convex Combination}\label{sec:convex_combination}\hfill
...
\subparagraph{Convex Hull}\label{sec:convex_hull}\hfill

$\symup{conv}(T)$

Set of all Convex Combinations of a Set of Points
...
\textbf{Caratheodory's Theorem} \emph{
  Every Point in the Convex Hull of a Set $T \subset \reals^n$ can be expressed
  as a Convex Combination of $n + 1$ or fewer Points from $T$.
}

\textbf{Approximate Catheodory's Theorem} \emph{
  Given a Set $T \subset \reals^n$ with Diameter $1$, for every Point
  $x \in \symup{conv}(T)$ one can find Points $x_1, \ldots, x_k \in T$ for any
  Integer $k > 0$ such that
  \[
    \| x - \frac{1}{k}\sum_i^k x_i \|_2 \leq \frac{1}{\sqrt{k}}
  \]
}
...
\subsubsection{Transverse Vector Field}\label{sec:transverse_vector_field}

\emph{Divergence-free Vector Field}, \emph{Incompressible Vector Field} or
\emph{Solenoidal Vector Field}

Divergence is Zero at all Points in the Field
...
\paragraph{Indefinite Inner Product Space}
\label{sec:indefinite_inner_product}\hfill

Indefinite (\S\ref{sec:isotropic_quadratic_form})



\subparagraph{Krein Space}\label{sec:krein_space}\hfill
...
% ------------------------------------------------------------------------------
\subsection{Tensor Vector Space}\label{sec:tensor_vector_space}
% ------------------------------------------------------------------------------

``Tensors'' in Machine Learning are Elements of Tensor Vector Spaces

FIXME:

\fist Tensor (\S\ref{sec:tensor})

\fist Tensor (\S\ref{sec:linear_tensor})
...
\subsubsection{Immanant}\label{sec:immanant}


Femions have transition amplitudes that are Determinants of Matrices; Bosons
have transition amplitudes that are Permanents of Matrices
(\url{https://scottaaronson.blog/?p=2925})



\paragraph{Determinant}\label{sec:determinant}\hfill
...
\subparagraph{Minor}\label{sec:minor}\hfill
...
\subparagraph{Pseudo-determinant}\label{sec:pseudo_determinant}\hfill
...
\subparagraph{Wronskian}\label{sec:wronskian}\hfill



\paragraph{Permanent}\label{sec:permanent}\hfill
...
\paragraph{Costas Array}\label{sec:costas_array}\hfill

\subparagraph{Golomb Ruler}\label{sec:golomb_ruler}\hfill

Difference Sets (\S\ref{sec:difference_set})
...
%FIXME: move section, merge with tensor vector space ???
...
Element of the Field used to define a Vector Space (\S\ref{sec:vector_space})
