...
\fist a Qubit (\S\ref{sec:qubit}) is a Quantum System with a Two-dimensional
Hilbert Space (FIXME: clarify)
...
If $V = W$ and $\forall \vec{v},\vec{w} \in V, B(\vec{v},\vec{w}) =
B(\vec{w},\vec{v])$ then $B$ is a Symmetric Bilinear Map
(\S\ref{sec:symmetric_bilinear_map})
