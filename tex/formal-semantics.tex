...
in a Finite Heyting Algebra, if more than $\frac{2}{3}$ of the Statements obey
the Law of Excluded Middle then it must be a Boolean Algebra
...
\paragraph{Predicative Definition}\label{sec:predicative_definition}\hfill

not Self-referencing

%TODO: xref stratification ???
Predicativity entails building \emph{Stratified} (or \emph{Ramified}) Theories
where Quantification over a Type at one ``level'' results in Types at a new
higher level, e.g. Intuitionistic Type Theory (\S\ref{sec:intuitionistic_type})

in the Calculus of Constructions (\S\ref{sec:coc}), $\mathtt{Prop}$ is
\emph{Impredicative}
...
\emph{Self-referencing Definition} -- the Definition ``invokes'' (mentions or
Quantifies over) the Set being Defined \emph{or} (more commonly) a Set that
\emph{contains} the thing being Defined
...
2022 - Weatherby, Justie - \emph{Indexical AI} --
\url{https://www.journals.uchicago.edu/doi/abs/10.1086/717312}



\paragraph{Deixis}\label{sec:deixis}\hfill

\emph{Referential Indexicality}



\subparagraph{Demonstrative}\label{sec:demonstrative}\hfill
...
2021 - Meunier - \emph{Computational Semiotics}



\subsubsection{Algebraic Semiotics}\label{sec:algebraic_semiotics}

1999 - Goguen - \emph{An Introduction to Algebraic Semiotics, with Application
