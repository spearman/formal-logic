...
Real-valued Continuous Function (\S\ref{sec:realvalued_continuous})
...
can be realized as the Fundamental Group of some Space.
\cite{hatcher02}
...
can be Continuously Deformed (\S\ref{sec:homotopy}) into eachother
within the Space $X$.
...
Path with equal Initial and Terminal Points: Continuous Function
(\S\ref{sec:continuous_function}) $f$ from Unit Interval $[0,1]$ to
Topological Space $X$ where $f(0) = f(1)$

Two Loops $B_m$ and $B_n$ with the same Initial and Terminal Points
can be combined into a new Loop $B_m + B_n = B_{m+n}$.
\cite{hatcher02}
