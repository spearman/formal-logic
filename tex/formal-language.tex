...
A \emph{String}, $s$, is a Finite Sequence (\S\ref{sec:sequence}) over
a non-empty Finite Set of \emph{Symbols} (or \emph{Characters}),
$\Sigma$, called an \emph{Alphabet}:
\[
...
\emph{Expressions} (or \emph{Phrases} \S\ref{sec:lexical_analysis} or
\emph{Well-formed Formulas} \S\ref{sec:formula} depending on the
context).
...
Terminating (\S\ref{sec:normal_form}) is called \emph{Convergent} (or
...
Termination (\S\ref{sec:normal_form}) is Decidable for Finite Ground
