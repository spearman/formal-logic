...
all of Formal Language Theory as well as Symbolic Logic (Part
\ref{sec:formal_logic}) and Proof Theory (Part \ref{sec:proof_theory})
are carried out Syntactically. The term \emph{Syntax} may be used as a
synonym for \emph{Grammar}: that is a Set of Rules through which the
strings of a Language can be either \emph{Generated}
(\S\ref{subsec:generative_grammar}) or \emph{Recognized}
(\S\ref{subsec:determinative_grammar}).
...
\emph{Term Logic} (\S\ref{subsec:term_logic})).
...
allows Functions to calculate values of Affixes.
...
A \emph{Parser} analyzes an Expression according to the rules of a
Formal Grammar, generating a \emph{Data Structure} describing the
Syntax of the input. An outline of the process follows.
...
\emph{Abstract Reduction} or \emph{Abstract Rewrite Systems}. This is
simply
    \[(A,\rightarrow)\]
