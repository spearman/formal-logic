...
is a Binary Relation on those objects called the \emph{Reduction
  Relation}. This is equivalent to an \emph{Unlabelled State
  Transition System} (\S\ref{sec:state_transition_system}).
...
An ARS $(A, \rightarrow)$ is \emph{Confluent} if and only if:
\[
    \forall w,x,y \in A,
...
If an ARS is Strongly Normalizing and Weakly Confluent, then it is
Confluent.
