...
a Formal Language can be Decided by a Family of Boolean Circuits
...
The \emph{Concatenation} of two Languages, $S$ and $T$, is an Associative
...
\fist Term-rewriting System (\S\ref{sec:term_rewriting})

\url{https://fexpr.blogspot.com/2016/08/interpreted-programming-languages.html}:
\begin{quote}
  ... formal grammars and term-rewriting calculi, which are both term-rewriting
  systems but have nearly diametrically opposed purposes.
\end{quote}

\url{https://fexpr.blogspot.com/2015/05/numberless-differentiation.html#sec-ndif-caus}
...
\item[Linear Context-free Rewriting System] \hfill \\
  ...
  (\S\ref{sec:thread_automaton})\cite{villemonte02} %FIXME may need to link to
...
\emph{Regular Languages} (or \emph{Token-level Languages}) are more restricted
...
\fist $\omega$-regular Languages (\S\ref{sec:omega_regular}) -- $\omega$-Kleene
Closure of the Regular Langauges
...
A \emph{Term Rewriting System} is an Abstract Rewriting System with \emph{Terms}
(Formal Logic\S\ref{sec:term}) as Objects and a Rewrite Relation,
$\rightarrow_R$, given by the Set of \emph{Term Rewriting Rules}, $R$, of the
form:
\[
  l \rightarrow r
\]
where $l$ and $r$ are a pair of Terms. Such a Rule is \emph{Applied} to a Term
$s$ if $l$ matches a Subterm of $s$.

Termination (\S\ref{sec:rewrite_termination}) is Decidable for Finite Ground
Systems, but Undecidable for Systems with a single Rule consisting of a Linear
(\S\ref{sec:linear_term}) left-hand side, or for Systems with only Unary
Function Symbols. % FIXME clarify finite ground system
...
% ------------------------------------------------------------------------------
\subsection{Parallel Rewriting System}\label{sec:parallel_rewrite}
% ------------------------------------------------------------------------------
...
Gabbay-Pitts02 - \emph{A new approach to abstract syntax with variable binding}
...
\emph{Combinational Logic} or \emph{Time-independent Logic}
...
A \emph{Turing Machine} operates on an infinite \emph{Storage Tape}, which acts
...
(vanLeeuwen94) Machine Model of Computation (\S\ref{sec:computation_model}); cf.
...
in the definition of Interactive Proof Systems. %FIXME refs
...
a Word on a Path in a Kripke Structure (\S\ref{sec:kripke_structure}) is an
$\omega$-word over the Alphabet $2^{AP}$ where $AP$ are the Atomic Propositions
of the Kripke Structure

Operations:

Intersection, Union, Left Catenation, Omega (Infinite Iteration), Finite
Prefixes, Limits

Distance, Metric Space %FIXME

(Dube 1993) constructing an $\omega$-language from the Set of Contractive Affine
Transformations $\Sigma$ of an IFS (\S\ref{sec:ifs}) on a Metric Space $(X, d)$
...
$\omega$-Kleene Closure of the Regular Languages (\S\ref{sec:regular_language})

Recognized by the following classes of $\omega$-automata
(\S\ref{sec:omega_automaton}):
\begin{itemize}
  \item B\"uchi Automata
  \item Rabin Automata
  \item Streett Automata
  \item Parity Automata
  \item Muller Automata
\end{itemize}

precisely the $\omega$-languages definable a particular Monadic Second-order
Logic called $\mathsf{S1S}$
...
or \emph{Stream Automaton}

variant of Finite Automaton (\S\ref{sec:finite_automaton}) that runs on Infinite
(rather than Finite) Input Strings (Words)

the following classes of Automata recognize the same class of
$\omega$-languages, that is the $\omega$-regular Languages

not expected to Terminate-- different ``Acceptance Conditions'' required:
\begin{itemize}
  \item \emph{B\"uchi Condition} ($\omega$-regular Languages
    \S\ref{sec:buchi_automaton}) -- Terminates if a Final State is visited
    Infinitely often
  \item \emph{Rabin Condition}
  \item \emph{Streett Condition} -- Negation of the Rabin Condition
  \item \emph{Parity Condition}
  \item \emph{Muller Condition}
\end{itemize}

may be Deterministic or Non-deterministic

all except B\"uchi Automata recognize precisely the Regular $\omega$-languages
and differ only in Acceptance Conditions; some may be more succinct in their
representations of a given $\omega$-language
...
Recognizer for Subset of $\omega$-regular Languages (\S\ref{sec:omega_regular});
strictly weaker than other $\omega$-automata

\emph{B\"uchi Condition}: Terminates if a Final State is visited Infinitely
often

used in Model Checking (\S\ref{sec:model_checking}, Kripke Structures
\S\ref{sec:kripke_structure}) as an Automata-theoretic version of a Formula in
Linear Temporal Logic (\S\ref{sec:linear_temporal})

every B\"uchi Automaton can be regarded as a Muller Automaton
(\S\ref{sec:muller_automaton})

class of Deterministic B\"uchi Automata is strictly weaker (TODO: explain)
...
every B\"uchi Automaton (\S\ref{sec:buchi_automaton}) can be regarded as a
Muller Automaton
