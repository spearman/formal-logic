...
\emph{Formal Logic} (or \emph{Symbolic Logic}) utilizes Formal
Language (Part \ref{sec:formal_language}) to develop \emph{Inferences}
(\S\ref{sec:inference_rules}) from a Set of zero or more \emph{Axioms}
(\S\ref{sec:axiomatization}). The Rules of Inference and Axioms form a
\emph{Deductive Apparatus} (\S\ref{sec:deductive_apparatus}).
