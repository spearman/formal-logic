...
\begin{itemize}
  \item Propositions as Session Types (\S\ref{sec:session_type})
...
\begin{itemize}
  \item Session (\S\ref{sec:session})
  \item Parallel Composition: $P_1 | P_2$
  \item Name Hiding: $(\nu a)P \quad (\nu k)P$
  \item Conditional: $\text{if } e \text{ then } P \text{ else } Q$
    with Boolean Expression $e$
  \item Recursion: $\mathtt{def }X_1(\tilde{x}_1 \tilde{k}_1) = P_1
    \mathtt{ and } \cdots \mathtt{ and } X_n(\tilde{x}_n \tilde{k}_n)
    = P_n \mathtt{ in } P$
\end{itemize}
...
\begin{itemize}
  \item \emph{Send}
  \item \emph{Receive}
  \item \emph{Sequence} (not needed) %FIXME
  \item \emph{Choice}
  \item \emph{Recursion}
\end{itemize}
...
Namespaces: \cite{honda-vasconcelos-kubo98}
\begin{itemize}
...
Atomic Interactions:
\begin{itemize}
  \item \emph{Data Sending/Receiving} (\S\ref{sec:data_passing}):
    Value Passing, including Name Passing (Synchronous Message Passing
    as in $\pi$-calculus \S\ref{sec:pi_calculus})
  \item \emph{Label Selection/Branching}
    (\S\ref{sec:label_branching}): Label Branching (Method Invocation
    without Value Passing)
  \item \emph{Channel Sending/Receiving} (\S\ref{sec:delegation}):
    Delegation (Passing a Channel to another Process)
\end{itemize}
...
\cite{honda-vasconcelos-kubo98}
\begin{itemize}
  \item Value Passing, including Name Passing (Synchronous Message
    Passing as in $\pi$-calculus \S\ref{sec:pi_calculus})
  \item Label Branching (Method Invocation without Value Passing)
  \item Delegation (Passing a Channel to another Process)
\end{itemize}


Combining Communication Primitives:
\begin{itemize}
  \item Concurrent (Parallel) Composition
...
Label Selection, Label Branching
...
Recursion-free $pi$-calculus and Finite-control $pi$-calculus (number
of Parallel components in any Process is Bounded by a Constant)
...
\subparagraph{Replicated Input}\label{sec:replicated_input}
\hfill \\
...
Replicated Input Processes (\S\ref{sec:replicated_input_process}) such
as $!x(y).P$ can be understood as \emph{Servers} waiting on Channel
$x$ to be Invoked by \emph{Clients}
...
\paragraph{Asynchronous $\pi$-calculus}\label{sec:asynchronous_pi_calculus}
\hfill \\
