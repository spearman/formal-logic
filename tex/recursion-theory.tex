...
\subsubsection{Deadlock}\label{sec:deadlock}
...
Environments $\Gamma, \Delta, \Theta$: associating Names to
...
$A \parr B$ Session Type of a Process that Inputs an $A$ then behaves
as $B$
...
$A \oplus B$ Session Type of a Process that Selects from either an $A$
or a $B$

$A \& B$ Session Type of a Process that Offers a Choice of either an
$A$ or a $B$
...
\subsubsection{Calculus of Communicating Systems (CCS)}
\label{sec:ccs}
