...
Systems is Left-adjoint (\S\ref{sec:adjoint}) to a Functor
...
(Q,L,\rightarrow,q_0)$, $R$ is $ST_L$: the Synchronization Trees
(\S\ref{sec:synchronization_tree}) over Labels $L$; the Final
...
Taking $L = X \times Y$, $ST_L$ is the Space $R$ of Relations extended
in Time, $R = \pow((X \times Y) \times R)$
...
$S_A \subseteq^{\text{nepref}} \Sigma_A^*$ -- Safety Specification: a
Non-empty Prefix-closed Subset of $\Sigma_A^*$
...
\[
  p;q \equiv (p \times q) [((a,b),(b,c)) \mapsto (a,c)]
\]
