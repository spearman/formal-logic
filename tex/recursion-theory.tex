...
1993 - Dube - \emph{Undecidable Problems in Fractal Geometry} -- connection
between Computability Theory and Fractal Geometry (Part
\ref{part:fractal_geometry});
...
cf. Attractors (\S\ref{sec:attractor_repeller}) of Iterated Function Systems
(\S\ref{sec:ifs}) \fist Fractal Geometry (\S\ref{part:fractal_geometry})
...
for an Endomorphism $f : X \rightarrow X$ of a Finite Set $X$, the Eventual
Image $im^\infty(f)$ of $f$ is the Set of Periodic Points of $f$
--\url{https://golem.ph.utexas.edu/category/2022/10/the_eventual_image_eventually.html}
...
cf. \emph{Decidable Set} (\S\ref{sec:decidable_set})

\fist Constraint Satisfaction Problem (\S\ref{sec:constraint_satisfaction})

Fundamental (Undecidable) Decision Problems in Group Theory
(\S\ref{sec:group_theory}):
\begin{itemize}
...
\url{https://statmodeling.stat.columbia.edu/2024/01/30/minimum-criteria-for-studies-evaluating-human-decision-making/}



\subsubsection{Undecidable Problem}\label{sec:undecidable}

\paragraph{Entscheidungsproblem}\label{sec:entscheidungsproblem}\hfill

\paragraph{Halting Problem}\label{sec:halting_problem}\hfill

\paragraph{Post Correspondence Problem}\label{sec:post_correspondence}\hfill

simpler than Entscheidungsproblem and Halting Problem
...
(\S\ref{sec:algorithm}); that is a Function that is \emph{Effectively
Calculable} by an Effective Method (\S\ref{sec:effective_method}).

\emph{Computable Functions} may be broadly divided into two Classes by whether
they are \emph{Partial} or \emph{Total}. A Total Computable Function is
sometimes called a (Total) \emph{Recursive} or \emph{Decidable Function}.

There are several equivalent definitions of the class of Computable Functions,
which have reference to different \emph{Models of Computation}
...
cf. Universality (Analysis \S\ref{sec:universality})
...
1993 - Dube - \emph{Undecidable Problems in Fractal Geometry} --
Computationally Universal, Fractal-based (Part \ref{part:fractal_geometry}:
Fractal Geometry) Geometric Model of Computation

Decidable Algorithm of an IFS with Affine Transformations Halts on an Input if
no Intersection is found; the Attractor then represents a graphical encoding of
the Strings that are \emph{not} in the Recognized Language
...
Dana Scott - Set $D$ Isomorphic to Function Space $D \rightarrow D$ of Functions
on itself can be defined if only Continuous Functions
(\S\ref{sec:continuous_function}) are considered, providing a Model for
$\lambda$-calculus. cf. Denotational Semantics
...
Differentiation \fist Differential $\lambda$-calculus
(\S\ref{sec:differential_lambda})
...
$\mathsf{MIP} = \mathsf{NEXPTIME}$ -- the Class of Problems Solvable by Multiple
Independent Provers is equal to the Class of Problems Solvable by a
Non-deterministic Machine in Exponential Time; also all ``Languages'' in
$\mathsf{NP}$ have Zero-knowledge Proofs (\S\ref{sec:zero_knowledge_proof}) in
an $\mathsf{MIP}$ System

$\mathsf{MIP}^* = \mathsf{RE}$ -- the Class of Problems Solvable by Multiple
Quantumly-entangled Provers is equal to the Class of Recursively Enumerable
Problems (that includes the Halting Problem); i.e. Entangled Provers can
convince a Polynomial-time Verifier that an arbitrary Turing Machine Halts
...
\subsubsection{Interaction Protocol}\label{sec:interactive_protocol}
...
all ``Languages'' in $\mathsf{NP}$ have Zero-knowledge Proofs in an
$\mathsf{MIP}$ System
...
% ------------------------------------------------------------------------------
\subsection{Average-case Complexity}\label{sec:average_case_complexity}
% ------------------------------------------------------------------------------

% ------------------------------------------------------------------------------
\subsection{Worst-case Complexity}\label{sec:worst_case_complexity}
% ------------------------------------------------------------------------------
...
\subsubsection{Double Exponential}\label{sec:double_exponential_time}
...
% ------------------------------------------------------------------------------
\subsection{Random Self-reducibility}\label{sec:random_self_reducibility}
% ------------------------------------------------------------------------------
...
\url{https://inst.eecs.berkeley.edu/~cs191/fa08/lectures/lecture17.pdf} --
\emph{Quantum Computation and Extended Church-Turing Thesis}: with suitable
Infinite-precision Operations, an Analog Computer can solve NP-Complete problems
in Polynomial Time
...
an \emph{Arithmetic Formula} is a Circuit in which every Node has Outdegree $1$,
and so the underlying Graph is a Directed Tree (\emph{Polytree}
\S\ref{sec:polytree})
...
% ------------------------------------------------------------------------------
\subsection{Descriptive Complexity Theory}\label{sec:descriptive_complexity}
% ------------------------------------------------------------------------------
...
Formalization: Linear Actegories (\S\ref{sec:linear_actegory})

Denotational Semantics: Power Domains (\S\ref{sec:power_domain})

Chu Spaces (\S\ref{sec:chu_space}) -- Models of Concurrent Computation in
Automata Theory to express ``Branching Time'' and Concurrency
...
-- Topological Semantics (\S\ref{sec:topological_semantics})
...
\fist cf. \emph{Process (Cybernetics)} (\S\ref{sec:process_cybernetics})
...
Fano Plane (\S\ref{sec:fano_plane})
...
\fist cf. Stochastic Petri Nets (\S\ref{sec:stochastic_petri_net})
...
-- Geometry of Interaction is the Optic (\S\ref{sec:optic}) for Co-pointed
Functors
