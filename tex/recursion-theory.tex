...
\subsubsection{$\mu$-recursion}\label{sec:mu_recursion}
...
\emph{Untyped $\lambda$-calculus} can be seen as a Typed
$\lambda$-calculus with a single Type (\S\ref{sec:type}).

Untyped $\lambda$-calculus uses three constructions:
...
\subsubsection{Computation Rule}\label{sec:computation_rule}

\[
    (\lambda x.t)(u) :\equiv t[u/x]
\]

\subsubsection{Alpha Equivalence}\label{sec:alpha_equivalent}

\subsubsection{Capture-avoiding Substitution}\label{sec:capture_avoiding}
...
\subsubsection{Normalization \& Confluence}
