...
\end{align*}
Preserves all the Linear Category Structure. History-free strategies
suffice to Interpret Linear Types.
...
$p \circ q : A \rightarrow C$

\[
  p \circ q \equiv (p \times q) [((a,b),(b,c)) \mapsto (a,c)]
\]
...
\[
  ...
  {p \circ q \xrightarrow{(a,c)} p' \circ q'}
\]
...
Interpreted in Linear Categories (???)
...
Object up to canonical Isomorphism: $\ocircle$ has the Unique Fixpoint
Property (\S\ref{sec:unique_fixpoint}). Functors on $\cat{SProc}$
whose Action on Morphisms is \emph{Guarded} (???) will have the Unique
Fixpoint Property.
...
If $p$ and $q$ are Deterministic, $p \circ q$ need not be
