...
\[
  \mathtt{def} \; X_1(\tilde{x}_1 \tilde{k}_1)
  = P_1 \; \mathtt{and} \; \cdots \; \mathtt{and} \;
  X_n(\tilde{x}_n \tilde{k}_n) = P_n \; \mathtt{in} \; P
\]
...
Process (Program \S\ref{sec:process}): Collection of Dyadic Interactions
(Communications \S\ref{sec:communication}) in a Session
...
\cite{honda-vasconcelos-kubo98}:
\[
  \mathtt{request} \; a(k) \; \mathtt{in} \; P
  \quad\quad\quad
  \mathtt{accept} \; a(k) \; \mathtt{in} \; P
\]
...
\subsubsection{Classical Processes}\label{sec:classical_processes}
...
\[
  \mathtt{accept} \; a(k) \; \mathtt{in} \; k![1];
  k?(y) \; \mathtt{in} \; P
  \quad | \quad
  \mathtt{request} \; a(k) \; \mathtt{in} \; k?(x)
  \; \mathtt{in} \; k![x+1]; \mathtt{inact}
\]
...
\begin{enumerate}
  ...
  \item $P \;|\; \mathtt{inact} \equiv P$, \quad\quad $P | Q \equiv Q
    | P$, \quad\quad $(P|Q)|R \equiv P|(Q|R)$
  \item $(\nu u) \; \mathtt{inact}$, \quad
    $(\nu u u)P \equiv (\nu u)P$, \quad
    $(\nu u u')P \equiv (\nu u' u)P$, \\
    $u \notin fu(Q) \Rightarrow (\nu u)P | Q \equiv (\nu u)(P|Q)$, \\
    $u \notin fu(D) \Rightarrow
    (\nu u) \mathtt{def} \; D \; \mathtt{in} \; P
    \equiv \mathtt{def} \; D \; \mathtt{in} \; (\nu u)P$
  \item $fpv(D) \cap fpv(Q) = \varnothing \Rightarrow
    (\mathtt{def} \; D \; \mathtt{in} \; P) | Q
    \equiv \mathtt{def} \; D \; \mathtt{in} \; (P|Q)$
  \item $fpv(D) \cap fpv(D') = \varnothing \Rightarrow
    \mathtt{def} \; D \; \mathtt{in} \;
    (\mathtt{def} \; D' \; \mathtt{in} \; P)
    \equiv \mathtt{def} \; D \; \mathtt{and} \; D' \mathtt{in} \; P$
\end{enumerate}
...
\textbf{Example of a Reduction (Synchronous)} \cite{honda-vasconcelos-kubo98}
...
\begin{split}
  P ::=&\quad \mathbf{0} \quad
      ...
      | \quad x(y).P \quad
      | \quad !x(y).P \quad \\
      |&\quad x.\mathtt{inl}; P \quad
      | \quad x.\mathtt{inr}; P \quad
      | \quad x.\mathtt{case}(P,Q)
\end{split}
\]
...
\begin{itemize}
  \item $x.\mathtt{inl}; P$
  \item $x.\mathtt{inr}; P$
  \item $x.\mathtt{case}(P,Q)$
\end{itemize}

Free Names $fn(P)$, Closed Process has no Free Names
...
\begin{tabular}{l l}
  ...
  & $(R!)$ \\
  $x.\mathtt{inl}; P | x.\mathtt{case}(Q,R) \rightarrow P | Q$
  & $(RL)$ \\
  $x.\mathtt{inr}; P | x.\mathtt{case}(Q,R) \rightarrow P | R$
  & $(RR)$ \\
...
\begin{split}
  \alpha ::=&\quad x(y) \quad
           ...
           |&\quad x.\mathtt{inl} \quad
           | \quad x.\mathtt{inr} \quad
           | \quad \overline{x.\mathtt{inl}} \quad
           | \quad \overline{x.\mathtt{inr}} \quad
\end{split}
\]
Side Conditions:
$(\mathtt{res})$
...
\begin{itemize}
  ...
  \item $x.\mathtt{inl}$ (Left Offer)
  \item $x.\mathtt{inr}$ (Right Offer )
  \item $\overline{x.\mathtt{inl}}$ (Left Select)
  \item $\overline{x.\mathtt{inr}}$ (Right Select)
...
\begin{split}
  \alpha  ::=&\quad \uparrow[\alpha]; \beta \quad
            | \quad \downarrow[\alpha]; \beta \quad \\
...
$\mathtt{request}$ and another with an $\mathtt{accept}$
...
$A \multimap B$: Session $x$ that first Inputs a Session Channel
(\S\ref{sec:session_channel}) of Type $A$ and then behaves as
Interactive Behavior (???) $B$

$A \otimes B$: first Sends a Session Channel of Type $A$ and
afterwards behaves as $B$ (Continuation Session); $A$ and $B$ are
independent

$!A$: Shared Server (\S\ref{sec:shared_server}) for Sessions of Type
$A$
