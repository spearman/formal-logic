...
cf. Fixed Points (\S\ref{sec:fixed_point})
...
(wiki): not Piecewise Differentiable but \emph{Non-chaotic} (Lyapunov Exponents
are Non-positive)
...
2000 - Rutten - \emph{Universal Coalgebra: a theory of systems} \cite{rutten00}
...
% ------------------------------------------------------------------------------
\subsection{Discrete Event Dynamical System (DEDS)}\label{sec:deds}
% ------------------------------------------------------------------------------
...
Time Constant $\tau$ -- main Characteristic Unit
...
cf. Fractal Geometry (Part \ref{part:fractal_geometry})
...
\emph{Observability} (\S\ref{sec:observability})
...
cf. Recursive Process (\S\ref{sec:process})
...
or \emph{System Function} or \emph{Network Function}
...
Cost Function (\S\ref{sec:objective_function})
...
PID Control (\S\ref{sec:pid_controller}) design tunes Parameters, Optimal
Control design is \emph{Non-parametric}
(\url{https://www.argmin.net/p/feedback-learning-and-adaptation})
...
\subparagraph{Certainty Equivalence}\label{sec:certainty_equivalence}\hfill

(wiki)
\emph{Certainty Equivalence Property}: in a Discrete-time Centralized
System (\S\ref{sec:centralized_system}) with Linear System (Evolution) Function,
Quadratic Cost Function, and Additive Noise, the Optimal Control Solution is the
same as would be obtained in the absence of Additive Disturbances
...
\subsubsection{Process}\label{sec:process_cybernetics}

\fist cf. Recursive Process (\S\ref{sec:process})

Feedback (\S\ref{sec:feedback})
