...
B$ with $a \in A ~ b \in B$ when there Exists $x \in C$ such that
...
special case of a Hughes Arrow (\S\ref{sec:hughes_arrow}) with ``output''
but no ``input''
...
A Monad also arises as a Composition $G \circ F$ of Adjoint Functors
$F : \cat{C} \rightarrow \cat{D}$ and $G : \cat{D} \rightarrow
...
\HandRight\; Cf. Non-symmetric Operad (\S\ref{sec:operad}) in
$\cat{Set}$
