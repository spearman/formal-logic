...
\subsubsection{Characteristic Morphism}\label{sec:characteristic_morphism}
...
$\cat{End_C}$ (or $\mathrm{End}(\cat{C})$) with Objects as
...
\subsubsection{$H$-algebra}\label{sec:h_algebra}

$T$-algebra (\S\ref{sec:t_algebra}) for a Monad without Associativity

Endofunctor $H : \cat{C} \rightarrow \cat{C}$

\emph{$H$-algebra} is an Object $X \in \cat{C}_0$ and a Morphism
$\alpha : H (X) \rightarrow X$ called the \emph{Carrier} of the
Algebra

The Category of $H$-algebras is equivalent to the Category of
$T$-algebras of the Free Monad (\S\ref{sec:free_monad}) on $H$. This
is analogous to the way a Binary Function (Sets) $A \times B
\rightarrow B$ is equivalent to a Monoid Action
(\S\ref{sec:monoid_action}) of the Free Monoid $A^*$ on $B$.



\subsubsection{$S$-algebra}\label{sec:s_algebra}
...
% --------------------------------------------------------------------
\subsection{Category Product}\label{sec:category_product}
% --------------------------------------------------------------------
...
% --------------------------------------------------------------------
\subsection{Monad}\label{sec:monad}
% --------------------------------------------------------------------
...
(\S\ref{sec:effect_system}) is Algebraic Effects
...
also an Algebra over a Pointed Endofunctor (\S\ref{sec:s_algebra})
...
\subsubsection{Idempotent Monad}\label{sec:idempotent_monad}
...
\subsubsection{Finitary Monad}\label{sec:finitary_monad}

or \emph{Algebraic Monad}

Finitary Lawvere Theory (\S\ref{sec:lawvere_theory})

Algebraic Theory (\S\ref{sec:algebraic_theory})



\subsubsection{Eilenberg-Moore Category}\label{sec:eilenberg_moore}
...
\subsubsection{Kleisli Category}\label{sec:kleisli_category}
...
\subsubsection{Monadic Adjunction}\label{sec:monadic_adjunction}
...
\subsubsection{Adjoint Category}\label{sec:adjoint_category}
...
$\mathrm{Mnd}(\cat{C})$: ``Monad Category''; Objects: Monads on
...
$\mathrm{PtEnd}(\cat{C})$ Pointed Endofunctors
(\S\ref{sec:pointed_endofunctor})

$\mathrm{End}(\cat{C})$ Category of Endofunctors of $\cat{C}$
...
\paragraph{Algebraically-free Monad}\label{sec:algebraically_free}
\hfill \\
...
\subsubsection{Codensity Monad}\label{sec:codensity_monad}
...
% --------------------------------------------------------------------
\subsection{Comonad}\label{sec:comonad}
% --------------------------------------------------------------------
...
\subsubsection{Cofree Comonad}\label{sec:cofree_comonad}
...
% --------------------------------------------------------------------
\subsection{Hughes Arrow}\label{sec:hughes_arrow}
% --------------------------------------------------------------------
...
\subsubsection{$A$-algebra}\label{sec:a_algebra}
\cite{jacobs-heunen-hasuo09}
...
$H$-algebra (Endofunctors \S\ref{sec:h_algebra}) \\
$S$-algebra (Pointed Endofunctors \S\ref{sec:s_algebra}) \\
$T$-algebra (Monads \S\ref{sec:t_algebra}) \\
...
\paragraph{Kan Complex}\label{sec:kan_complex}
\hfill \\
...
% --------------------------------------------------------------------
\subsection{Autonomous Category}\label{sec:autonomous_category}
% --------------------------------------------------------------------
...
\begin{itemize}
  ...
  \item If $T$ is a Dependent Type Theory (\S\ref{sec:dependent_type})
    ...
    is Locally Cartesian Closed (\S\ref{sec:locally_cartesian})
...
by Grothendieck Construction Equivalence %FIXME
