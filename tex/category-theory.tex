...
$\mathbf{D}$, can be established by a Pair of Functors
(\S\ref{sec:functor}):
\[
...
Faithful Functor (\S\ref{sec:faithful_functor}) called the
\emph{Inclusion Functor} which Maps Objects and Morphisms to
themselves:
\[
  I : \mathbf{S} \rightarrow \mathbf{C}
\]
If $I$ is a Full Functor then $S$ is a \emph{Full Subcategory}.
...
%FIXME merge with kernels?



\subsection{Coequalizer}\label{sec:coequalizer}
...
of an \emph{Endofunctor} which is a Functor from a Category to itself.
...
(\S\ref{sec:pointed_category}). For a Zero Object, $Z \in \mathbf{C}$,
there is a Unique Morphism between any two Objects, $A, B \in
\mathbf{C}$ called the \emph{Composite} in $Z$:
\[
...
\subsubsection{Cone}\label{sec:functor_cone}
...
Limits (\S\ref{sec:limit}) and Colimits (\S\ref{sec:colimit}) are
defined as Universal Cones.
...
A \emph{Limit} is defined as a Universal Cone
