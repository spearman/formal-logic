...
\S\ref{sec:internal_homfunctor} $[X,1]$) and a Morphism $1 \rightarrow
...
General Context for treating Dual Objects (\S\ref{sec:dual_object})

Autonomous Category (\S\ref{sec:rigid_category}) that is also
Symmetric (\S\ref{sec:symmetric_monoidal})

\emph{Rigid Symmetric Monoidal Category} with Internal Hom
(\S\ref{sec:internal_homfunctor}) given by:
\[
