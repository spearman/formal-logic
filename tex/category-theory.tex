...
\rightarrow A$ called the \emph{Evaluator} of the Algebra:
\cite{corfield08}
\[
  (A,\alpha)
\]
...
i.e. Least Fixed-point, or a Final Coalgebra
...
The Carrier of the Initial Algebra is the Fixed Point of $F$:
\[
  Fix (F) = Fx (F (Fix (F)))
\]
...
\]
where $alg$ is an Evaluator for an $F$-algebra.
...
Terminal Coalgebras $x$ are Fixed Points of their Endofunctors: $F x
\cong x$
