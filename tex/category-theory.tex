...
Categories (\S\ref{sec:category}) to equate Vertices with Objects
$G_0$ and Edges with Morphisms $G_1$.
...
\]\[
    D_0(cod(f)) = cod(D_1(f))
\]
Give two Graphs, $A$ and $B$, with the same Set of Objects, $O$, the
...
\begin{enumerate}
    \item $c : G \times_{G_0} G \rightarrow G$ Mapping Composable
      Morphisms to their Composite
    \item $i: G_0 \rightarrow G_1$ Mapping Objects to their Identity
      Functions
\end{enumerate}
...
The Forgetful Functor (\S\ref{sec:functor}) $U : \mathbf{Cat}
\rightarrow \mathbf{Grph}$ Maps a Categories to their underlying
Graphs, effectively forgetting which Morphisms are Identities and
...
\]
Vertical Composition is Associative and has an Identity.
...
% --------------------------------------------------------------------
\subsection{Congruence Category}\label{subsec:congruence_category}
% --------------------------------------------------------------------
...
\subsubsection{Quotient Category}\label{subsec:quotient_category}
