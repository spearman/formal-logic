...
(\S\ref{subsec:representable_functor}).
...
(\S\ref{subsec:setvalued_category}).



\subsubsection{Simplicial Set}\label{subsec:simplicial_set}

A \emph{Simplicial Set} is a Contravariant Functor from the
\emph{Simplex Category} $\Delta$ (\S\ref{subsec:categories_list}) into
...
first argument and Covariant in the second. See \emph{Representable
  Functor} \S\ref{subsec:representable_functor}.
...
\subsubsection{Monad}\label{subsec:monad}
...
\subsubsection{Products \& Coproducts}\label{subsec:universal_product}
...
(\S\ref{subsec:set_product}) of the two Sets.
...
(\S\ref{subsec:isomorphism}). The Canonical Commutative Isomorphism $A
...
Rule of Inference: %FIXME two-way inference
\[
    {
...
\paragraph{N-ary Products}\label{subsec:category_nary}
\hfill \\
...
\paragraph{Exponential}\label{subsec:category_exponential}
\hfill \\
...
two-way Rule of Inference: %FIXME
\[
    {
...
% --------------------------------------------------------------------
\subsection{Dependency Morphism}\label{subsec:dependency_morphism}
% --------------------------------------------------------------------

\emph{Trace Monoid} (\S\ref{subsec:trace_monoid})
...
(\S\ref{subsec:universal_product}) in $\mathbf{Cat}$.
...
\subsubsection{Set-valued Functor Category}\label{subsec:setvalued_category}

A \emph{Set-valued Functor Category} is also called a \emph{Category
  of Diagrams} and is of the form $\mathbf{Set}^{\mathbf{C}}$. A
Presheaf (\S\ref{subsec:presheaf}) is an example of a Set-valued
...
\subsubsection{Cartesian Closed Category}\label{subsec:cartesian_category}

A Category of \emph{Cartesian Closed} if it has:
\begin{enumerate}
\item A Product Functor (\S\ref{subsec:universal_product})
\item An Exponential for any pair of Objects
  (\S\ref{subsec:category_exponential})
\item A Terminal Object (\S\ref{subsec:initial_terminal})
\end{enumerate}
By these conditions and the Universal Mapping Property of
Exponentials:
\[
    \forall f : X \times A \rightarrow B \iff
    \exists ! \overline{f} : X \rightarrow B^A
\]

The Category of Types in the \emph{Simply Typed $\lambda$-calculus}
(\S\ref{subsec:simply_typed}) is a Catesian Closed Category.
...
\subsubsection{Trace Monoid}\label{subsec:trace_monoid}
...
Formula (\S\ref{subsec:propositional}).
