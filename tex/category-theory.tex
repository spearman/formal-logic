...
A \emph{Hom-functor} Maps between a given Locally Small Category,
$\mathbf{C}$, and $\mathbf{Set}$, and has a Covariant and a
...
\begin{enumerate}
    \item \emph{Covariant Hom-functor}, for $A,f : X \rightarrow Y \in
      \mathbf{C}$:
\[
    Hom(A,-) : \mathbf{C} \rightarrow \mathbf{Set}
\]\[
    X \mapsto Hom(A,X)
\]\[
...
\]
    ...
    \item \emph{Contravariant Hom-functor} (also \emph{Functor of
      Points}, see \S\ref{subsec:general_element}), for $B,h : X
      \rightarrow Y \in \mathbf{C}$:
\[
    Hom(-,B) : \mathbf{C} \rightarrow \mathbf{Set}
\]\[
    X \mapsto Hom(X,B)
\]\[
    h \mapsto Hom(h,B) : Hom(Y,B) \rightarrow Hom(X,B)
\]
    where $Hom(h,B)$ is defined for all $g \in Hom(Y,B)$ as:
\[
    g \mapsto g \circ h
\]
\end{enumerate}
