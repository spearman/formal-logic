...
$\cat{End_C}$ (or $\mathrm{End}(\cat{C})$)i with Objects as
...
\]
  ...
  \item \emph{Contravariant Hom-functor} (also \emph{Functor of
    ...
    \S\ref{sec:generalized_element}), for $B,f : X \rightarrow Y \in
    \cat{C}$:
\[
...
\paragraph{$A$-algebra}\label{sec:a_algebra}
\cite{jacobs-heunen-hasuo09}
\hfill \\
...
\paragraph{Geometric Category}\label{sec:geometric_category}
\hfill \\
...
\paragraph{Heyting Category}\label{sec:heyting_category}
\hfill \\
...
\begin{itemize}
  ...
  \item If $T$ is a Dependent Type Theory (\S\ref{sec:dependent_type})
    with Dependent Product Types (\S\ref{sec:dependent_product}) then
    $Syn(T)$ is Locally Cartesian Closed
