...
Category Theory as Proof-relevant Order Theory
...
by Syntax-Semantics Duality one may view Type Theory as a Formal Syntactic
Language (or \emph{Calculus}) for Category Theory, and conversely one may think
of Category Theory as providing Semantics for Type Theory (Part
...
\item [CGHaus] all Separated (Hausdorff) Spaces (\S\ref{sec:separated_space})
  and (???) Continuous Maps (Full Subcategory of $\cat{CGTop}$)
...
\item [Man] or $\cat{Diff}$ all Smooth Manifolds (\S\ref{sec:smooth_manifold})
  ...
  with Points of $T X$ being Ordered Pairs $(x, v)$ where $v$ is a \emph{Tangent
    Vector} at $x$
\item [Cob] (or $\cat{nCob}$) all Closed Manifolds and Cobordisms
...
An \emph{Equivalence} between two Categories, $\cat{C}$ and $\cat{D}$,
$\cat{C} \simeq \cat{D}$, can be established by a Pair of Functors
(\S\ref{sec:functor}):
\[
...
\fist Note Equality of Categories $\simeq$ is distinct from Isomorphism of
Categories $\cong$.

Equivalence and Isomorphism of Categories stand in the same relation as Homotopy
Equivalences and Homeomorphisms of Topological Spaces (Loregian 2015)
...
A \emph{Thin Category} (or \emph{Posetal Category}) has at most one Morphism
between Objects, e.g. a Preorder (a Poset without Antisymmetry
\S\ref{sec:preorder}).
...
% ------------------------------------------------------------------------------
\subsection{Restriction Category}\label{sec:restriction_category}
% ------------------------------------------------------------------------------

\subsubsection{Turing Category}\label{sec:turing_category}

a Cartesian (\S\ref{sec:cartesian_category}) Restriction Category

2014 - Cockett, Hofstra, Hrubes

\url{https://golem.ph.utexas.edu/category/2019/11/total_maps_of_turing_categorie.html}
...
\]
and has the property for any two Morphisms $g, h : C \to A$, $fg = fh$ Implies
$g = h$ ($f$ is Left Cancellative \S\ref{sec:cancellative_property}). A
Monomorphism in a Category $\cat{C}$ is an Epimorphism in $\cat{C^{op}}$.

Equivalently, $f : A \to B$ is a Monomorphism if for every Object $X$,
$Hom(X,f) : Hom(X,A) \to Hom(X,B) : u \mapsto f \circ u$ is Injective.
...
Functions. In $\cat{Set}$ Monomorphisms are exactly the Injective Functions
between Sets. This is also true in the Categories $\cat{Grp}$, $\cat{Rng}$, and
any Abelian Category (\S\ref{sec:abelian_category}).

Equalizers (\S\ref{sec:equalizer}) are always Monomorphisms and as such are used
to define the notion of Subobject (\S\ref{sec:subobject}).

\fist Note not all Monomorphisms are required to be Injective.

Left-invertible Morphisms are necessarily Monomorphisms (Split Monomorphism
...
\]
and has the property for any two Morphisms $g, h : B \to C$, $gf = hf$ Implies
$g = h$ ($f$ is Right Cancellative \S\ref{sec:cancellative_property}). An
Epimorphism in a Category $\cat{C}$ is a Monomorphism in $\cat{C^{op}}$.

Equivalently, $f : A \to B$ is an Epimorphism if for every Object $Y$,
$Hom(f,Y) : Hom(B,Y) \to Hom(A,Y) : u \mapsto u \circ f$ is Injective.
...
\fist Note that being an Epimorphism doesn't imply the Morphism is Surjective.
It is more frequently True for Monomorphisms that Injective Functions imply they
are Monomorphism than the reverse for Surjective Functions and Epimorphisms.
Regular (\S\ref{sec:regular_epimorphism}) and Strong
(\S\ref{sec:strong_epimorphism}) Epimorphisms behave closer to Surjections than
ordinary Epimorphisms.

If $f$ is an Epimorphism, then $fg$ and $fh$ are Idempotent
(\S\ref{sec:idempotent}). %FIXME correct ???

\fist Parts of a System are associated with (Regular) Epimorphisms (Fong et al.
2018 \emph{Behavioral Mereology} \S\ref{sec:behavioral_type}) --
...
A Bimorphism $f : A \rightarrow B$ is an \emph{Isomorphism} if there exists
another Morphism $g : B \rightarrow A$ such that $f \circ g = 1_Y$ and $g \circ
f = 1_X$, i.e. $f$ admits a Two-sided Inverse. The Existance of an Isomorphism
between two Objects $A$ and $B$ is denoted $A \cong B$.

\fist Note that a Bimorphism need not be an Isomorphism.

In a Concrete Category (\S\ref{sec:concrete_category}) an Isomorphism must
correspond to a Bijective Function on the underlying Sets, but there are
Concrete Categories with Bijective Morphisms that are not necessarily
Isomorphisms (e.g. in $\cat{Top}$) and there are also Categories with Objects
corresponding to Sets with Isomorphisms that are not Bijections.

The Existence of an \emph{Invertible Morphism} (\S\ref{sec:bijective_function})
between Objects also Implies their Isomorphism. A Category in which every
Morphism is an Isomorphism is a Groupoid (\S\ref{sec:groupoid}). A Category in
which every Morphism is an Isomorphism and which has a single Object is a Group
(\S\ref{sec:group}).

If an Inverse of a Morphism exists, then it is unique.

\begin{itemize}
  \item In $\cat{Top}$ the Category of Topological Spaces and Continuous Maps,
    the Isomorphisms are exactly the Homeomorphisms
...
a list of some Isomorphisms:
\begin{itemize}
  \item every $n$-dimensional Vector Space over a Field $F$ is Isomorphic to the
...
the Set of all Endomorphisms with Composition forms an \emph{Endomorphism
  Monoid} (\S\ref{sec:endomorphism_monoid})

\begin{itemize}
  \item \emph{Transformation} (\S\ref{sec:transformation}) -- Set Endomorphism
  \item ...
\end{itemize}
...
an \emph{Automorphism} is both an Endomorphism and an Isomorphism, that is, an
Invertible (Bijective \S\ref{sec:bijective_function}) Endomorphism

an Automorphism of an Algebraic Structure is called a \emph{Symmetry}
(\S\ref{sec:structure_symmetry})

the Set of all Automorphisms of an Object forms an \emph{Automorphism Group}
(\S\ref{sec:automorphism_group})

\begin{itemize}
  \item \emph{Permutation} (Bijective Transformation \S\ref{sec:permutation})
    -- Set Automorphism
  \item \emph{Linear Transformation} (\S\ref{sec:linear_transformation})
    -- Module Automorphism
  \item ...
\end{itemize}
...
\url{https://www.epatters.org/wiki/algebra/monoid-objects.html}
...
Power Object of $C$ consists of an Object $\Omega^C$ and a Monomorphism
(\S\ref{sec:monomorphism}) $\in_C \hookrightarrow C \times \Omega^C$ such that
for every other Object $D \in \cat{C}_0$ and every Monomorphism $R
\hookrightarrow C \times D$ there is a Unique Morphism $\chi_R : D \rightarrow
\Omega^C$ such that $R$ is the Pullback of:
\[
...
A Category with Finite Limits and Power Objects for all Objects is precisely a
Topos (\S\ref{sec:topos}).
...
\fist cf. Differential Categories (\S\ref{sec:differential_category})
...
% ------------------------------------------------------------------------------
\subsection{Coproduct Category}\label{sec:coproduct_category}
% ------------------------------------------------------------------------------
...
A \emph{Slice Category} (or \emph{Overcategory}) $\cat{C}/C$ of a Category
$\cat{C}$ with an Object $C$ has as Objects the Morphisms with Codomain $C$ and
as Morphisms those Morphisms in $\cat{C}$ between the Domains of the underlying
Morphisms of the Objects of $\cat{C}/C$. That is, for Objects in the Slice
Category corresonding to Morphisms $f$ and $f'$, the Morphism in the Slice
Category between the two is $g$ such that
\[
  f' \circ g = f
\]
...
\subsubsection{Bundle Category}\label{sec:bundle_category}
...
% ==============================================================================
\section{Filtered Category}\label{sec:filtered_category}
% ==============================================================================

generalization of Directed Set (\S\ref{sec:directed_set})

the Colimit of a Functor $F : J \to C$ where $J$ is a Filtered Category is
called a \emph{Filtered Colimit} (\S\ref{sec:filtered_colimit})

a Category $J$ is Cofiltered if the Opposite Category $J^{op}$ is Filtered



% ------------------------------------------------------------------------------
\subsection{Ind-completion}\label{sec:ind_completion}
% ------------------------------------------------------------------------------

process of freely adding Filtered Colimits (\S\ref{sec:ind_completion}) to a
given Category

Objects of an Ind-completed Category are called \emph{Direct Systems}
(\S\ref{sec:direct_system})



\subsubsection{Direct System}\label{sec:direct_system}

or \emph{Ind-object}

Object of a Ind-completed Category



% ------------------------------------------------------------------------------
\subsection{Pro-completion}\label{sec:pro_completion}
% ------------------------------------------------------------------------------

dual of Ind-completion

$Pro(C) := Ind(C^{op})^{op}$



\subsubsection{Inverse System}\label{sec:inverse_system}

\emph{Projective System} or \emph{Pro-object}

Object of a Pro-completed Category

Functors $F : I \to C$ from a Co-filtered Category $I$
...
A \emph{Bifunctor} is a Functor of two Variables from a Product Category to an
arbitrary Category:
\[
...
A \emph{Profunctor} (or \emph{Distributor}) is a Bifunctor that is Contravariant
in the first argument and Covariant in the second.

Generalization of Functors, Categorical generalization of Bimodules
(\S\ref{sec:bimodule})

Compositional nature of Profunctors is the natural ``Categorification'' of
...
\subsubsection{Profunctor Optics}\label{sec:profunctor_optics}

\emph{Profunctor Encoding} of \emph{Optics} (\S\ref{sec:optic}) --
\emph{Bidirectional Data Accessors} (\S\ref{sec:bidirectional_transformation})

$\mathbf{Optic}(\cat{C})$

``lawless'' Lenses

%TODO: move lens section under this heading ?
...
\emph{Parametric Optics} (\S\ref{sec:parametric_optics}) -- nlab: Para
Construction (\S\ref{sec:para_construction}) $Para(Optic(C))$ recovers the
Category of Neural Networks (Capucci et al. 2020)

\asterism

2018 - Riley - \emph{Categories of Optics}

Category of Lenses: Optic Category for a Symmetric Monoidal Category with Tensor
given by Binary Product
...
A \emph{Presheaf} is a Contravariant Functor from an Opposite Category to the
Category $\cat{Set}$. A Presheaf is an example of a Set-valued Functor Category
(\S\ref{sec:category_diagram}) and gives a Cartesian Closed Category
...
Escardo, Simpson 2001 -
\emph{A Universal Characterization of the Closed Euclidean Interval}

\asterism

2020 - Leinster - \emph{The categorical origins of Lebesgue integration}

Universal Properties that characterize the Lebesgue $L^p$ Spaces
(\S\ref{sec:lp_space})

Banach Space $L^p[0,1]$, $1 \leq p < \infty$ is \emph{Initial}
...
A \emph{Category Diagram} is a Covariant Functor from an \emph{Index Category}
...
A \emph{Cone} in a Diagram $D : \cat{J} \to \cat{C}$ is an Object $C \in
\cat{C}_0$ and a Unique Morphism $c_j : C \to D_j$ for each Object in the
Diagram (i.e. an Initial Object) such that any resulting triangles Commute.

This is equivalent to a Natural Tranformation from the Constant Functor
(\S\ref{sec:constant_functor}) $\Delta_C$ to the Diagram Functor $D$.

\emph{Left Cone}: $\cat{J}^\vartriangleleft$ -- Category obtained by adding a
single Initial Object $-\infty$
...
A Limit (\S\ref{sec:limit}) is a Terminal Object in the Cone Category.



\subsubsection{Cocone}\label{sec:cocone}

A \emph{Cocone} in a Diagram $D : \cat{J} \rightarrow \cat{C}$ is an Object $C
\in \cat{C}_0$ and a Unique Morphism $c_j : D_j \rightarrow C$ for each Object
in the Diagram (i.e. a Terminal Object) such that any resulting triangle
Commutes.

\emph{Right Cone}: $\cat{J}^\vartriangleright$ -- Category obtained by adding a
single Terminal Object $\infty$
...
\emph{Wedge}, $X \in \cat{D}$ with Family of Morphisms $\omega_C : X \rightarrow
T(C,C)$ in $\cat{D}$ for all $C \in \cat{C}$ such that for any $f : C
\rightarrow C'$ in $\cat{C}$:
\[
  \omega_C \circ T(1_C,f) = \omega_C' \circ T(f,1_{C'})
\]
(Extranatural Transformation \S\ref{sec:extranatural_transformation}

$\omega_C(X) = n_C : C \rightarrow C$ are Components of a Natural Transformation
from $1_\cat{C} \rightarrow 1_\cat{C}$. A Wedge $X \xrightarrow{.} Hom :
\cat{C}^{op} \times \cat{C} \rightarrow \cat{Set}$ is a Function $X \rightarrow
Nat (1_\cat{C},1_\cat{C})$.
...
\fist Decorated Cospan (Hypergraph Categories \S\ref{sec:decorated_cospan}),
Category of Cospans (\S\ref{sec:cospan_category})

a Cospan is a pair of Morphisms $X \rightarrow N \leftarrow Y$ with a common
Codomain (Apex)
...
a Cobordism (\S\ref{sec:cobordism}) $(W; M, N)$ is a kind of Cospan
...
\emph{End} of a Functor is a Terminal Wedge (\S\ref{sec:wedge}) $E
\xrightarrow{.} T$ where $T : \cat{C}^{op} \times \cat{C} \rightarrow \cat{D}$
and $E \in \cat{D}$

$\int_{C \in \cat{C}} T(C,C)$

cf. Trace of a Matrix except with Product instead of Sum

The End of $Hom : \cat{C}^{op} \times \cat{C} \rightarrow \cat{Set}$ is
$Nat (1_\cat{C},1_\cat{C})$ called the \emph{Hochschild Cohomology}
(\S\ref{sec:hochschild_homology}) or \emph{Symmetries} of $\cat{C}$.

For two Functors $F,G : \cat{C} \rightarrow \cat{E}$, the End for the Functor
$\cat{E}(F(-), G(-)) : \cat{C}^{op} \times \cat{C} \rightarrow \cat{Set}$ is:
\[
  \int_{C \in \cat{C}} \cat{E}(F(C), G(C)) = Nat (F,G)
\]

Forgetful Functor, Tannakian Reconstruction (\S\ref{sec:tannakian_category})
%FIXME tannakian reconstruction
Monoid $(M,\mu,\eta)$, $\int_{(M,\mu,\eta)} \cong M$

2015 - Loregian - \emph{Coend calculus}

$T : \cat{C}^{op} \times \cat{C}^{op} \rightarrow \cat{D}$
...
\subsubsection{Filtered Limit}\label{sec:filtered_limit}

Filtered Colimit (\S\ref{sec:filtered_colimit})
...
\begin{itemize}
  \item a Profinite Group (\S\ref{sec:profinite_group}) is a Topological Group
    that is Isomorphic to the Inverse Limit of an Inverse System of Discrete
    Finite Groups
\end{itemize}
...
a Coend is an Initial Wedge ... (Loregian 2015)

of a Functor $S : \cat{C}^{op} \times \cat{C} \rightarrow \cat{X}$, a Pair $(d,
\zeta)$ with Object $d \in \cat{X}_0$ and Extranatural Transformation
(\S\ref{sec:extranatural_transformation}) $\zeta : S \xrightarrow{..} d$

``Integral'' over a Category

cf. Trace of a Matrix

2015 - Loregian - \emph{Coend calculus}

a Functor $T : \cat{C}^{op} \times \cat{C}^{op} \rightarrow \cat{D}$ can be
thought of as a generalized form of evaluation of an Object $C$ against another
and the ``quantity'' $T(C,C')$ can be ``Integrated'' to yield two distinct
Objects having Dual Universal Properties:
\begin{itemize}
  \item a \emph{Coend} $\int^C T(C, C)$, by ``Symmetrization'' along the
    Diagonal of $T$ (FIXME: explain)
  \item an \emph{End} $\int_C T(C, C)$, an Object arising as an ``Object of
    Invariants'' of ``Fixed Points'' for the same Action of $T$ on Arrows
    (FIXME: explain)
\end{itemize}
by Dualization, if a Coend is a Quotient of $\coprod_{C\in\cat{C}}T(C, C)$, an
End is a Subobject of the Product $\prod_{C\in\cat{C}}T(C, C)$

all Co/ends can be characterized as Co/limits (but they provide a ``richer'' set
of computational rules than mere Co/limits)

given $T : \cat{C}^{op} \times \cat{C} \to \cat{D}$, there exists a Category
$\bar{\cat{C}}$ and a Functor $\bar{T} : \bar{\cat{C}} \to \cat{D}$ such that:
\[
  \int^C T(C,C) \cong \mathrm{colim}_\bar{\cat{C}}\bar{T}
\]
and
\[
  \int_C T(C,C) \cong \mathrm{lim}_\bar{\cat{C}}\bar{T}
\]

Coends ``Categorify'' Logical Calculus -- Coend Calculus provides a
Higher-dimensional version of a Deductive System suited for Category Theory

the Integral notation is motivated by the fact that Co/ends ``behave like
Integrals'' in that the Fubini Rule (FIXME: Fubini Theorem ???) of exchange
holds (TODO: clarify)

in a sense, the Theory of Left Kan Extensions along the Yoneda Embedding
embodies ``all'' Category Theory
...
\fist a \emph{Tensor} (\S\ref{sec:tensor_calculus}) is a collection of Vectors
and Covectors (Linear Forms \S\ref{sec:linear_form}) using the Tensor Product

(wiki): by the Universal Property (\S\ref{sec:universal_property}) of the Tensor
Product there is a canonical correspondence between \emph{Bilinear Forms}
(\S\ref{sec:bilinear_form}) on $V$ and \emph{Linear Maps} $V \otimes V \to K$
...
\subsubsection{Filtered Colimit}\label{sec:filtered_colimit}

Colimit of a Functor $F : J \to C$ where $J$ is a Filtered Category
(\S\ref{sec:filtered_category})
...
An Object that is both an Initial and a Terminal Object is called a \emph{Zero
Object} (or \emph{Null Object}). A Category with a Zero Object is called a
\emph{Pointed Category} (\S\ref{sec:pointed_category}).

For a Zero Object, $Z \in \cat{C}$, there is a Unique Morphism between any two
Objects, $A, B \in \cat{C}$ called the \emph{Composite} in $Z$:
\[
...
both Product (\S\ref{sec:product}) and Coproduct (\S\ref{sec:coproduct})
...
Compact Closed Categories with Biproducts -- Categorical Semantics of Quantum
Logic (Abramsky, Coecke 2004)

(Cockett et al. 2019) every Category with Finite Biproducts is a Cartesian
Differential Category (\S\ref{sec:cartesian_differential}) where every Map is
Linear (by definition of the Differntial Combinator); in a Cartesian
Differential Category, the Subcategory of Linear Maps has Finite Biproducts
...
\url{https://twitter.com/jer_gib/status/1252678411220332544}:

Adjunctions $F \dashv G$ and $F' \dashv G'$ Compose to
$F \circ F' \dashv G' \circ G$

every Adjunction $F \dashv G$ and Monad $M$ gives rise to a Monad
$G \circ M \circ F$ \fist Monad Transformers (\S\ref{sec:monad_transformer});
cf. StateT (Haskell)
...
2020 - Pavlovic, Hughes - \emph{The nucleus: Mining concepts from adjunctions}
...
Right Adjoint Functors Preserve \emph{Limits} (\S\ref{sec:limit}) and
Monomorphisms (\S\ref{sec:monomorphism})

Left Adjoint Functors Preserve \emph{Colimits} (\S\ref{sec:colimit}) and
...
Lawvere 1969 - Existential and Universal Quantification as Left and Right
...
\fist cf. Call-By-Push-Value (CBPV \S\ref{sec:cbpv}); every Monad comes from an
Adjunction between a Language with a Side Effect, and a Language without one
...
van Laarhoven Function: $(X \rightarrow F X) \rightarrow F Y$ equivalent to
Store Comonad; cf. Lenses (\S\ref{sec:lens}) in Programming, Extensible Effects
(\S\ref{sec:extensible_effect})
...
For Functor $J : \cat{A} \rightarrow \cat{C}$, the \emph{Codensity Monad} is the
Right Kan Extension (\S\ref{sec:kan_extension}) $\mathrm{Ran}_J J$ of $J$ along
itself, if it exists (when $\cat{A}$ is Small and $\cat{C}$ is Complete)

$J$ is Codense (\S\ref{sec:codense_functor}) just when its Codensity Monad is
the Identity (Codensity Monad measures the failure of $J$ to be Codense)

\fist Measurable Cardinals (\S\ref{sec:measurable_cardinal})

\url{https://www.maths.ed.ac.uk/~tl/ucr/}:

produces concepts such as:
\begin{itemize}
  \item Prime Ideals and Fields of Fractions
  \item Radical of an Integer
  \item Ultrafilters and Ultraproducts
  \item Compact Hausdorff Spaces with Sheaves
  \item Double Dualization of Vector Spaces
  \item Linearly Compact Vector Spaces
...
Integration Operators (\S\ref{sec:integral_operator}); the Set $T(X)$ of
Integration Operators on a Set $X$ is a Functor $T : \cat{Set} \to \cat{Set}$
and is the Codensity monad of the Inclusion
$\cat{Ctbl} \hookrightarrow \cat{Set}$ of Countable Sets into the Category of
Sets
...
-- Probability Monad (\S\ref{sec:probability_monad})
...
Monads give classes of Functions that in some way ``enlarge'' the possible
Function Outputs (Effects)

Comonads give classes of Functions that in some way ``enlarge'' the possible
Function Inputs (Context-dependencies)

$Extract : T(X) \rightarrow X$ -- (Monad $Return : X \rightarrow T(X)$)
...
Categorical Compositional Distributional Models (Clark et al. 2010)

\url{https://golem.ph.utexas.edu/category/2019/06/meeting_the_dialogue_challenge.html}
...
\fist Closed Symmetric Monoidal Categories
...
\fist Turing Category -- a Cartesian Restriction Category
(\S\ref{sec:restriction_category})

2021 - Cruttwell et al. - \emph{Categorical Foundations of Gradient-based
Learning} -- Category of Lenses (\S\ref{sec:lens}) in a Cartesian Category
$\cat{C}$: $\mathbf{Lens}(\cat{C})$

\begin{itemize}
  \item Cartesian Differential Category (\S\ref{sec:cartesian_differential})
\end{itemize}
...
Actegories (\S\ref{sec:actegory})



\subparagraph{Totally Distributive Category}
\label{sec:totally_distributive_category}\hfill

\subparagraph{Linearly Distributive Category}\label{sec:linearly_distributive}



% ------------------------------------------------------------------------------
\subsection{Actegory}\label{sec:actegory}
% ------------------------------------------------------------------------------
...
TODO: the following definition had $\cat{X}$ be a Linearly Distributive
Category; does it still work for a general Distributive Category ?

Symmetric Monoidal Category $\cat{A}$

Distributive Category $\cat{X}$

Functors $\circ : \cat{A} \times \cat{X} \to \cat{X}$ and
$\bullet :  \cat{A}^{op} \times \cat{X} \to \cat{X}$ such that $\circ$ is the
Parameterized Left-adjoint of $\bullet$



\subsubsection{Linear Actegory}\label{sec:linear_actegory}

Formalization of Concurrent Computation (\S\ref{sec:concurrent_computation})

2007 Cockett, Pastro - \emph{The Logic of Message Passing}

\url{https://golem.ph.utexas.edu/category/2023/09/linear_actegories.html}
...
Para Construction (\S\ref{sec:para_construction}) for a Symmetric Monoidal
Category $\cat{C}$: $\mathbf{Para}(\cat{C})$--if $\cat{C}$ is Non-strict, this
gives a Bicategory (\S\ref{sec:bicategory})
...
Resource Theory
(\url{https://golem.ph.utexas.edu/category/2023/09/concurrent_process_histories_a.html})

2014 - Baez, Erbele - \emph{Categories in Control} -- Signal-flow Diagrams
(\S\ref{sec:signal_flow}) as String DiagramsA for the Symmetric Monoidal
Categori $\cat{FinVec}_k$ of Finite-dimentional Vector Spaces over the Field of
Rational Functions

2019 - Shulman -
\emph{A Practical Type Theory for Symmetric Monoidal Categories}



\paragraph{Commutative Monoidal Category}\label{sec:commutative_monoidal}\hfill

Tensor Product is Strictly Associative and Unital and also Strictly Commutative
...
\subparagraph{Cosmos}\label{sec:cosmos}\hfill

a Symmetric Closed Monoidal Category that is Complete and Co-complete
...
\paragraph{Copy-discard Category}\label{sec:cd_category}\hfill

or \emph{Garbage-share Monoidal Category}

Symmetric Monoidal Category where every Object is equipped with a distinguished
Commutative Comonoid structure and the Comonoid structures are compatible with
the Tensor Products such that for $\forall$ Objects $X$ and $Y$:
\[
  \text{copy}_{X\otimes{Y}} =
    (id_X \otimes b_{Y,X} \otimes id_Y) (\text{copy}_X \otimes \text{copy}_Y)
\]
where $b$ denotes the Braiding



\subparagraph{Affine Copy-discard Category}\label{sec:affine_cd_category}\hfill

(nlab):

or \emph{Markov Category} is a Copy-discard Category where any of the following
equivalent conditions hold:
\begin{itemize}
  \item the Monoidal Unit $I$ is a Terminal Object (i.e. the Category is
    Semicartesian Monoidal)
  \item the Discard (Counit) maps $\text{de]: X \to I$ are part of a Natural
    Transformation $\id \Rightarrow I$
  \item for every Morphism $f : X \to Y$, $\text{del}_Y \circ f = \text{del}_X$
\end{itemize}

interpretations:
\begin{itemize}
  \item Information Theory: ``processing'' any Information and discarding the
    result is the same as discarding the input altogether
  \item Probability Theory: Probabilities are Normalized to $1$
\end{itemize}
...
$(\cat{C}, \otimes, I)$ is \emph{Compact Closed} if every Object
$A \in \cat{C}_0$ has a Dual Object (\S\ref{sec:dual_object}).

General Context for treating Dual Objects

Compact Closed Categories are precisely the Symmetric Autonomous (Rigid)
Categories (\S\ref{sec:rigid_category}).
...
Compact Closed Categories with Biproducts (\S\ref{sec:biproduct}) -- Categorical
Semantics of Quantum Logic (Abramsky, Coecke 2004)

\begin{itemize}
  \item (Monoidal) Reverse Differential Categories
    (\S\ref{sec:reverse_differential_category}) -- a Differential Category which
    is also Self-dual Compact Closed
  \item ...
\end{itemize}
...
(wiki): a purely Combinatorial construction designed to capture the notion of a
``Well-behaved'' Topological Space for use in Homotopy Theory
(\S\ref{sec:homotopy_theory})

cf. Simplicial Complex (\S\ref{sec:simplicial_complex}) -- a Set of
$n$-dimensional Simplices (\S\ref{sec:simplex})

2011 - Friedman -
\emph{An elementary illustrated introduction to simplicial sets}
...
\paragraph{Globular Set}\label{sec:globular_set}\hfill

Globular Sets are to Simplicial Sets as Globes (\S\ref{sec:cellular_n_globe})
are to Simplices
...
(\S\ref{sec:compactly_generated}) Separated (Hausdorff) Topological Spaces
...
% ==============================================================================
\section{Cocategory}\label{sec:cocategory}
% ==============================================================================

\url{https://twitter.com/FellowHominid/status/1607365932133003266}
...
Replace Hom-\emph{sets} with Objects from a general Monoidal Category
(\S\ref{sec:monoidal_category}) $\cat{K}$, allowing \emph{Hom-objects} to be
something other than Sets of Functions, such as Vector Spaces or Topological
Spaces of Morphisms. The Category $\cat{K}$ must be Monoidal so that Composition
...
a Category Enriched over Truth Values is a Preorder (\S\ref{sec:preorder}) --
...
Category Enriched over the Monoidal Category (\S\ref{sec:monoidal_category}) of
Abelian Groups $\cat{Ab}$, i.e. Category $\cat{C}$ is Preadditive if every
Homset $Hom(A,B)$ in $\cat{C}$ has the structure of an Abelian Group and
Composition of Morphisms is Bilinear (\S\ref{sec:bilinear_map}).
...
\begin{itemize}
  \item Differential Category (\S\ref{sec:differential_category})
  \item ...
\end{itemize}
...
% ------------------------------------------------------------------------------
\subsection{Differential Category}\label{sec:differential_category}
% ------------------------------------------------------------------------------

\emph{(Monoidal) Differential Categories}

Axiomatizes Linear Maps and derives Smooth Maps from them-- cf. Cartesian
Differential Category (\S\ref{sec:cartesian_differential}) Axiomatizes Smooth
Maps and derives Linear Maps from them

\fist cf. Differential Object (\S\ref{sec:differential_object})

2006 - Blute, Cockett, Seely - \emph{Differential Categories}

Semi-additive (\S\ref{sec:semiadditive_category}) Symmetric Monoidal
(\S\ref{sec:symmetric_monoidal}) Category $\cat{X}$ with a Comonad (Co-algebra
Modality), $(S, \delta, \epsilon)$, and a \emph{Differential Combinator}:
\[
  D_\otimes : \cat{X}(S(A),B) \to \cat{X}(A \otimes S(A), B)$
\]
satisfying four Axioms ... TODO

example: a Comonad $S_\infty$ on the Category of Vector Spaces -- fully captures
usual notion of Derivatives of Smooth Maps

\fist Differential $\lambda$-calculus (\S\ref{sec:differential_lambda})

Objects in a Differential Category should be thought of as Spaces with a
``Modality'' (a Comonad) and the Morphisms should be thought of as Linear, and
the Co-Kleisli Maps for the Modality should be interpreted as being
\emph{Smooth} (Differentiable)

a Smooth Map is a Linear Map $f : !A \to B$

in general the Type of the Differential should be $D(f): !A \to A \multimap B$
-- this is the case if the Monoidal Category is \emph{Closed}

Differential Combinator $D[f] : A \otimes !A \to B$ ``mediates'' the interaction
between the underlying Category of Linear Maps and the (Co-Kleisli) Category of
Differentiable Functions

2019 - Blute et al. - \emph{Differential Categories Revisited}

a Differential Category is an Semi-additive Symmetric Monoidal Category with a
Coalgebra Modality $(!, \delta, \varepsilon, \Delta, e)$ which comes equipped
with a ``Deriving (Natural) Transformation'' $d : !A \otimes A \to !A$



\subsubsection{Cartesian Differential Category}
\label{sec:cartesian_differential}

Cartesian (\S\ref{sec:cartesian_category})

Cartesian Forward Differential Category (Blute et al.)

$D[f] : A \times A \to B$

Cartesian Reverse Differential Category (Cockett et al.)

$D[f] : A \times B \to A$

2009 - Blute et al. - \emph{Cartesian Differential Categories}

directly Axiomatize Differentiable (Smooth) Maps and derives Linear Maps from
them

primary example: Category with Objects as Euclidean Spaces and Morphisms are
Smooth Maps

a Cartesian Left-additive Category is a Left-additive Category which is
Cartesian and such that all Projection Maps are Additive; alternatively a
Cartesian Left-additive Category is a Cartesian Category in which each Object
$A$ has the structure of a Commutative Monoid with Addition
$+_A : A \times A \to A$ and Zero $0_Z : 1 \to A$

since not every Morphism in a Cartesian Left-additive Category is Additive, the
Product is \emph{not} a Coproduct, thus not a Bi-product

a \emph{Cartesian Differential Category} is a Left-semi-additive Category with a
Cartesian Differential Operator that satisfies a number of equations, in
particular that the Differential of any map is ``Linear'' in a suitable sense

under modest and natural assumptions, the Co-Kleisli Category of a Differential
Category is Cartesian Differential

example: Category of Smooth Maps where Objects are Natural Numbers and Maps
$n \to m$ are Smooth Maps $\reals^n \to \reals^m$; the Product on Objects is
given by the Sum of Natural Numbers and the Additive structure is given by the
Sum of Smooth Functions

example of a Left-semi-additive Category: Category with Objects as
Finite-dimensional Real Vector Spaces and Morphisms are (Infinitely)
Differentiable Maps; has a natural ``Differential Operator'' given by the
Jacobian

Additive Maps form a Subcategory in a Left-semi-additive Category

\emph{Cartesian Differential Operator} $D_\times$ is a Family of Operators:
\[
  D_\times : \cat{X}(X,Y) \implies \cat{X}(X \times X \to Y)
\]

satisfying Axioms ... TODO

a Morphism in a Cartesian Differential Category is \emph{Linear} if
$D_\times[f] = \pi_0 f$

in a Cartesian Differential Category, every Linear Map is Additive

2018 - Cruttwell - \emph{Cartesian Differential Categories Revisited}

Genralized Cartesian Differential Categories

every Cartesian Category has an associated Cofree Differential Category

2019 Cockett et al.:

every Category with Finite Biproducts is a Cartesian Differential Category

in the Category of Smooth Functions, the Derivative
$D[F] : \reals^n \times \reals^n \to \reals^m$
of a Smooth Function $F : \reals^n \to \reals^m$ is defined as:
\[
  D[F](\vec{x},\vec{v}) \defeq J_F(\vec{x}) \cdot \vec{v}
\]
where $J_F(\vec{x})$ is the Jacobian of $F$ at $\vec{x}$

a Morphism $f$ in a Cartesian Differential Category is \emph{Linear}$ when
$D[f] = \pi_1 f$

in a Category with Finite Biproducts, \emph{every} Map is Linear by definition
of the Differential Combinator

in a Cartesian Differential Category, the Subcategory of Linear Maps has Finite
Biproducts



\paragraph{Tangent Category}\label{sec:tangent_category}\hfill

2014 - Cockett, Cruttwell -
\emph{Differential Structure, Tangent Strucutre, and SDG}

Synthetic Differential Geometry (\S\ref{sec:synthetic_differential_geometry})

every Cartesian Differential Category gives rise to a Tangent Category



\subsubsection{Reverse Differential Category}
\label{sec:reverse_differential_category}

2022 - Cruttwell et al. - \emph{Monoidal Reverse Differential Categories}

a \emph{Monoidal Reverse Differential Category} is an Additive Symmetric
Monoidal Category $\cat{X}$ such that $\cat{X}$ is a Self-dual Compact-closed
Category equipped with a Co-algebra Modality $(!, \delta, \varepsilon, \Delta,
e)$ and a Reverse Deriving Transformation which is a Family of Maps:
\[
  r_A : !A \otimes !A \to A
\]
with Axioms: ... TODO

a Reverse Differential Category is exactly a Differential Category which is also
Self-dual Compact Closed

if $\cat{X}$ is a Reverse Differential Category, then $\cat{X}$ is a
Differential Category where the Driving Transformation
$d_A : !A \otimes A \to !A$ is defined as:
\[
  d_A \defeq (\cup_A \otimes 1_{!A}) \circ (r_A \otimes \sigma_{!A,A})
    \circ (1_{!A} \otimes \cap_{!A} \otimes 1_A
\]


if $\cat{X}$ is a Differential Category which is also Self-dual Compact Closed,
then $\cat{X}$ is a Reverse Differential Category with Reverse Deriving
Transformation:
\[
  r_A \defeq (\cup_{!A} \otimes 1_A) \circ (d_A \otimes \sigma_{A,!A})
    \circ (1_{!A} \otimes \cap_A \otimes 1_{!A})
\]

the Category $\cat{Rel}$ of Sets and Relations is a Reverse Derivative Category
where the Reverse Deriving Transformation is the Relation that relates two Bags
that differ by one Element to that said Element

the Category $\cat{FinVec}_{\ints_2}^\mathcal{B}$ of Finite-dimensional Vector
Spaces over Field $\ints_2$ with Objects are pairs $(V, B_V)$ of
Finite-dimensional Vector Spaces over $\ints_2$ and a Basis of $V$, and
Morphisms are arbitrary $\ints_2$-linear Maps between underlying Vector Spaces,
is a Reverse Differential Category

the Co-Kleisli Category of a Reverse Differential Category is a Cartesian
Reverse Differential Category



\paragraph{Cartesian Reverse Differential Category}
\label{sec:cartesian_reverse_differential}\hfill

2020 - Cockett et al. - \emph{Reverse Derivative Categories}

a Category with a Reverse Derivative also has a Forward Derivative, but not vice
versa

a Reverse Derivative is equivalent to a Forward Derivative with a Dagger
Structure on its Subcategory of Linear Maps; having such a Dagger Structure is
enough to ensure that a Cartesian Differential Category gives a Cartesian
Reverse Differential Category

a Cartesian Lef-additive Category $\cat{X}$ has Reverse Derivatives if there is
a \emph{Reverse Differential Combinator} $R$:
\[
  R : \cat{X}(A,B) \implies \cat{X}(A \times B, A)
\]
satisfying Coherence conditions (Axioms): ... TODO

a \emph{Cartesian Reverse Differential Category} is a Cartesian Left-additive
Category with a Reverse Differential Operator

the Category of Smooth Maps is a Reverse Differential Category with Differential
Combinator defined using the Transpose of the Jacobian; for a Smooth Function
$F : \reals^n \to \reals^m$, its Reverse Derivative
$R[F] : \reals^n \times \reals^m \to \reals^n$ is defined as:
\[
  R[F](\vec{x},\vec{y}) \defeq (J_F(\vec{x}))^T \cdot \vec{y}
\]

if $\cat{X}$ is a Cartesian Reverse Differential Category with Reverse
Differential Combinator $R$, then $\cat{X}$ is a Cartesian Differential Category
with Differential Combinator $D$:
\[
  D[f] \defeq \pi_1 \circ R[R[f]] \circ (\langle{1, 0}\rangle \times 1)
\]

$\dagger$ structure: $f^\dagger \defeq \iota_1R[f]$

$f$ is Linear if and only if $f^{\dagger\dagger} = f$

a Cartesian Reverse Differential Category is a Cartesian Differential Category
for which the associated Linear Fibration is a Dagger Fibration in which each
Fibre has $\dagger$-biproducts (TODO: clarify)

2022 - Cruttwell et al. -
\emph{Categorical Foundations of Gradient-based Learning} -- a Neural Network is
a Map in the Category $\mathbf{Para}(\cat{Smooth})$
(\S\ref{sec:para_construction}):
\[
  f : \reals^p \times \reals^n \to \reals^m
\]
where $\reals^p$ is the Space of Parameters for the Neural Network; together
with the Reverse Derivative Operator as a Parameterized Lens:
$\mathbf{Para}(\mathbf{Lens}(\cat{Smooth}))$
...
\subsubsection{$n$-globe}\label{sec:cellular_n_globe}

(nlab): Globes are to Simplices as Globular Sets (\S\ref{sec:globular_set}) are
to Simplicial Sets
...
\begin{itemize}
  \item Profunctor Bicategory (\S\ref{sec:profunctor_bicategory})
  \item Cartesian Bicategory (\S\ref{sec:cartesian_bicategory})
  \item Decorated Cospan Bicategory (\S\ref{sec:decorated_cospan_bicategory})
\end{itemize}

Cospans (\S\ref{sec:cospan}) can be thought of as $1$-morphisms in a Bicategory
(Fong16 => Benabou67)
...
\paragraph{Para Construction}\label{sec:para_construction}\hfill

for Symmetric Monoidal Category $(\cat{C}, I, \otimes)$,

$\mathbf{Para}(\cat{C})$

is a Bicategory with:
\begin{itemize}
  \item 0-cells are Objects of $\cat{C}$
  \item 1-cells $A \to B$ is a ``choice'' of a Parameter Object $P : \cat{C}$
    and a Morphism $f : P \otimes A \to B$ in $\cat{C}$
  \item 2-cells $(P, f) \Rightarrow (Q, g)$ is a Morphism $r : P \to Q$ in
    $\cat{C}$ such that $f = g \circ (r \otimes A)$
\end{itemize}

Parametric Optics (\S\ref{sec:parametric_optics})
...
where $\reals^p$ is the Space of Parameters for the Neural Network; together
with the Reverse Derivative Operator as a Parameterized Lens:
$\mathbf{Para}(\mathbf{Lens}(\cat{Smooth}))$ \fist Cartesian Reverse
Differential Categories (\S\ref{sec:cartesian_reverse_differential})

Deep Learning, Game Theory

when the base Category is the Category of Optics (\S\ref{sec:profunctor_optic})



% ------------------------------------------------------------------------------
\subsection{$n$-category}\label{sec:n_category}
% ------------------------------------------------------------------------------

\subsubsection{$n$-functor}\label{sec:n_functor}

\paragraph{Lax $n$-functor}\label{sec:lax_n_functor}\hfill

a Lax 2-functor for which all the Structure 2-morphisms are Invertible is called
a \emph{Pseudofunctor} (\S\ref{sec:pseudofunctor})

2019 - Hedges - \emph{Lax Functors Describe Emergent Effects} -
\url{https://julesh.com/2019/12/02/lax-functors-describe-emergent-effects/}:

when describing ``Behavior'' via a Pseudofunctor to $\cat{Rel}$,
``\emph{Emergent Effects}'' (or ``\emph{Generative Effects}'') are described by
the feailure of the Functor to be ``\emph{Strong}'', i.e. to be an actual
Functor

example:

a Category $\cat{C}$ with Morphisms as Open Systems (e.g. a Hypergraph
Category), and Objects as ``Open Boundaries'' that a System can have, and
\emph{Composition} describes coupling two Systems along a common Boundary (e.g.
by Decorated or Structured Cospans)

define an Operation $F : \cat{C} \rightarrow \cat{Rel}$ describing the
``Behavior'' such that every Object (Boundary) $x$ is sent to the Set $F(x)$ of
\emph{Behaviors} the Boundary can ``exhibit'', and every Morphism
$f : x \rightarrow y$ is Sent to a Subset $F(f) \subseteq F(x) \times F(y)$,
such that $(a, b) \in F(f)$ says that given a System $f$, it is possible to
\emph{simultaneously observer} the Behavior $a$ on its ``left'' Boundary, and
Behavior $b$ on its ``right'' Boundary

Composition of Systems $f : x \rightarrow y$ and $g : y \rightarrow z$ yields
the \emph{Complex System} $f g : x \rightarrow z$ with its own Set of Behaviors
$F(fg) \subseteq F(x) \times F(z)$ and Behaviors on the outer Boundaries that
arise from Behaviors of Systems that are \emph{consistent} (agreeing on the
middle Boundary Behavior $F(y)$) are exactly described by Composition in
$\cat{Rel}$:
\[
  F(f) F(g) = \{(a,c) | (a,b) \in F(f) \wedge (b,c) \in F(g) \wedge b \in F(y)\}
\]
this condition is $F(f) F(g) \subseteq F(fg)$ is that $F$ is a
\emph{Lax Pseudofunctor}, thinking of $\cat{Rel}$ as a $2$-category

if $F(f) F(g) = F(fg)$ \emph{always}, then $F$ \emph{is} a Functor

failure of a Lax Functor to be a Functor is caused by \emph{Emergent Effects},
i.e. Behaviors of a Complex System that \emph{do not} come from possible
Behaviors of the component Systems

an \emph{Emergent Behavior} of a Complex System $f g$ is an Element of the Set:
\[
...
\paragraph{Oplax $n$-functor}\label{sec:oplax_n_functor}\hfill



\subsubsection{$2$-category}\label{sec:2_category}
...
\paragraph{Doctrine}\label{sec:doctrine}\hfill
...
\subparagraph{Gabriel-Ulmer Duality}\label{sec:gabriel_ulmer_duality}
\hfill
...
\subparagraph{Hyperdoctrine}\label{sec:hyperdoctrine}\hfill
...
\paragraph{Syntactic Category}\label{sec:syntactic_category}\hfill
...
\paragraph{Pseudofunctor}\label{sec:pseudofunctor}\hfill

Weak $2$-functor between Bicategories (\S\ref{sec:bicategory}):
a Lax 2-functor (\S\ref{sec:lax_n_functor}) for which all the Structure
...
\subparagraph{$2$-presheaf}\label{sec:2_presheaf}\hfill
...
\paragraph{Indexed Category}\label{sec:indexed_category}\hfill
...
Stable Homotopy Theory (\S\ref{sec:stable_homotopy})
...
\emph{Model Structure} on a Category $\cat{C}$ has Classes of Morphisms:
\begin{enumerate}
...
ncatlab: an $\infty$-category for which any $j$-morphism such that $j>r$ is
...
% ------------------------------------------------------------------------------
\subsection{Geometric Higher Category}\label{sec:geometric_higher_category}
% ------------------------------------------------------------------------------

\url{https://golem.ph.utexas.edu/category/2023/03/an_invitation_to_geometric_hig.html}

an ordinary String Diagram is a Manifold $2$-diagram
...
Proofs:
\begin{itemize}
  \item Proof is Finitist and Constructive $\Rightarrow$ holds in any Topos
  \item Proof is Constructive $\Rightarrow$ holds in any Topos with a Natural
    Numbers Object (???)
...
Category of Sheaves (\S\ref{sec:sheaf}) on a Category $\cat{C}$; Grothendieck
Topos (\S\ref{sec:grothendieck_topos})

$\cat{Set}$; Grothendieck Topos: Category of Sheaves on the Point

$\cat{FinSet}$; Elementary Topos (\S\ref{sec:elementary_topos})

any Topos is a Regular Category (\S\ref{sec:regular_category}) and an Adhesive
Category (\S\ref{sec:adhesive_category})
...
Structure on a Category $\cat{C}$ that makes Objects of $\cat{C}$ behave like
...
way, one should think of it as the Topological Space reduced to just one
...
A \emph{Site} is a Category $\cat{C}$ together with a chosen Grothendieck
Topology $J$: $(\cat{C},J)$
...
\subsubsection{Sheave Category}\label{sec:sheave_category}
...
where Classical Logic is an Internal Logic for $\cat{Set}$, Intuitionistic
Logic is the Internal Logic for a Category of Sheaves
...
\subsubsection{Ringed Topos}\label{sec:ringed_topos}

\paragraph{Bohr Topos}\label{sec:bohr_topos}\hfill

Presheaf Topos (TODO)

a Topos associated to a Quantum Mechanical System (\S\ref{sec:quantum_system})
such that the Quantum Observables and Quantum States of the System (i.e. its
Quantum Probability Theory) are equivalently the Classical Observables and
Classical States in the Internal Logic (\S\ref{sec:internal_logic}) of the Bohr
Topos

Quantum Probability Theory (\S\ref{sec:quantum_probability}) as Classical
Probability Theory \emph{Internal} to the Bohr Topos of a given Quantum
Mechanical System
...
% ------------------------------------------------------------------------------
\subsection{Higher Topos Theory}\label{sec:higher_topos}
% ------------------------------------------------------------------------------

$\infty$-topos (\S\ref{sec:infinity_1_topos})

2006 - Lurie - \emph{Higher Topos Theory}

\fist Derived Algebraic Geometry (\S\ref{sec:derived_algebraic_geometry})

\fist Higher Category Theory (\S\ref{sec:higher_category})
...
\emph{Dagger Category} (or \emph{Involutive Category} although an Involutive
Category might have additional conditions) %FIXME
...
every Endomorphism Hom-set in a Dagger Category is a Monoid with Involution
...
Category of Dagger Categories $\cat{DagCat}$ is a Cartesian Closed Category
...
\begin{enumerate}
  \item For all $f,g \in \cat{C}_1$:
    ...
    \]
  \item Associator, Left and Right Unitors, and Braiding are all Unitary
\end{enumerate}
%FIXME xrefs
...
Dagger used to connect the Unit to the Counit so that for all $A \in \cat{C}$:
\[
...
Categorical Quantum Mechanics, Entangled States and Measurements (Abramsky,
Coecke)

\begin{itemize}
  \item $\cat{Cob}$ (or $\cat{nCob}$) -- Category of Closed Manifolds and
    Cobordisms with Disjoint Union as Tensor and ``reversal'' (FIXME: ???) of
    Closed Manifolds as Dagger
  \item ...
\end{itemize}
...
Bicategory (\S\ref{sec:bicategory})
...
Structured Cospans (TODO)

2020 - Cicala - \emph{Rewriting Structured Cospans} -- Enzyme Design:
\url{https://cheminf.imada.sdu.dk/novo-synergy/}
...
Universal Algebra, Algebraic Topology
