...
\mathbf{C}$, namely $G F$. If $F$ and $G$ are Inverses then the
...
Given a Monad $T$ on a Category $\mathbf{C}$, there is a Category of
Adjunctions (\S\ref{sec:adjoint}) that give rise to that Monad.

A Monad gives an Algebraic Theory (\S\ref{sec:algebra}) and an Algebra
...
$\mathbf{C}^T$ Category of $T$-algebras (\S\ref{sec:t_algebra}) for
Monad $T$ in Category $\mathbf{C}$

Freely Constructed

$A \mapsto \mu_A : T^2 A \rightarrow T A$ is a Functor from
$\mathbf{C}$ to $\mathbf{C}^T$

$\theta : T A \rightarrow A \mapsto A$ is a Functor from
$\mathbf{C}^T$ to $\mathbf{C}$
...
For Monad $T$ and Category $\mathbf{C}$, a \emph{Kleisli Category}
$\mathbf{C}_T$ (or $Kl(T)$) is equivalent to the Category of Free
$T$-algebras (\S\ref{sec:t_algebra})-- Full Subcategory
(\S\ref{sec:full_subcategory}) of Eilenberg-Moore Category
$\mathbf{C}^T$

Objects are Objects of $\mathbf{C}$ and Morphisms for a pair of
Objects $A$, $B$ are the Morphisms in $\mathbf{C}(A, T B)$
...
Monad $(T, \eta, \mu)$ where $T : \mathbf{C} \rightarrow \mathbf{C}$

$T$-algebra $(A, \theta)$ where $A \in \mathbf{C}$ and \emph{Action}
...
$\mathbf{Alg}(T)$ (or $\mathbf{C}^T$, see Eilenberg-Moore Category
...
Two Functors $F : \mathbf{C} \rightarrow \mathbf{D}$ and $U :
\mathbf{D} \rightarrow \mathbf{C}$ are \emph{Adjoints} when there is a
Natural Isomorphism:
\[
  \Phi : Hom_\mathbf{D}(F C,D) \cong Hom_\mathbf{C}(C,U D) : \Psi
\]
...
\[
  \eta : 1_\mathbf{C} \rightarrow U F
\]
...
Natural Transformation:
\[
  \epsilon : F U \rightarrow 1_\mathbf{D}
\]
...
: 1_\mathbf{C} \rightarrow U \circ F$ (Unit):

\begin{itemize}
\item For any $C \in \mathbf{C}$, $D \in \mathbf{D}$, $f : C
...
\begin{itemize}
\item For any $C \in \mathbf{C}$, $D \in \mathbf{D}$, $g : F C
...
(\S\ref{sec:eilenberg_moore}) $\mathbf{C}^T$ for Monad $T = GF$.
...
Objects: $(x,C)$ where $C \in \mathbf{C}_0$ and $x \in PC$
...
For a Category $\mathbf{C}$, a \emph{Pullback} (or \emph{Fiber
...
For a Monoidal Category (\S\ref{sec:monoidal_category}) $\mathbf{M}$,
a \emph{Tensor Product} is a Functor:
\[
  \otimes : \mathbf{M} \times \mathbf{M} \rightarrow \mathbf{M}
\]
...
$\mathbf{C}$ with a Tensor Product (\S\ref{sec:tensor_product})
$\otimes$:
\[
...
Horizontal Categorification gives $2$-categories
(\S\ref{sec:2_category})
...
\item $\mathbf{Rel}$ Category of Sets and Relations
\item $\mathbf{Cob}$ Category of Closed Manifolds and Cobordisms
\item $Spans(\mathbf{C})$ of $\mathbf{C}$ with Pullbacks
\item $CoSpans(\mathbf{C})$ of $\mathbf{C}$ with Pushouts
