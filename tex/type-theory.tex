...
Computations $T$ that Return Values in $X$: $T X$ for ``suitable''
...
Example Non-deterministic Boolean $\mathtt{or}$ Operator:
\[
  \mathtt{or}(\mathtt{return}\;true, \mathtt{return}\;false)
  : F \mathrm{Bool}
\]
...
\[
  \mathtt{or}_X : T X^2 \rightarrow T X
\]
...
(defined Universally) with Domain is a Free Model of the Algebraic
Theory of the Effects ``at hand'', and Range is the Programmer-defined
Model (???) of the Algebraic Theory and Extends a Programmer-defined
Map on Values.
...
\begin{itemize}
  ...
  \item Subset of Base Types: Arity Types $\alpha_1, \alpha_2, \ldots$
  \item Function Symbols $f : (\beta_1, \ldots, \beta_n) \rightarrow
    \beta$
  \item Relation Symbols $R : (\beta_1, \ldots, \beta_n)$
\end{itemize}

\emph{Base Terms} $v$: Variables $x,y,z,\ldots$, Function Symbols
$f (v_1, \ldots, v_n)$
