...
\paragraph{Tree}\label{sec:tree_type}\hfill
...
\paragraph{Mutually Recursive Data Type}
\label{sec:mutually_recursive_datatype}\hfill

\subparagraph{Forest}\label{sec:forest_type}\hfill

Tree (\S\ref{sec:tree_type})



\paragraph{Isorecursive Data Type}\label{sec:isorecursive_datatype}\hfill
...
\paragraph{Infinite Data Type}\label{sec:infinite_datatype}\hfill
...
\paragraph{Sum Type}\label{sec:sum_type}\hfill
...
\paragraph{Product Type}\label{sec:product_type}\hfill
...
\paragraph{Coproduct Type}\label{sec:coproduct_type}\hfill

%FIXME sum type?



\paragraph{Quotient Type}\label{sec:quotient_type}\hfill
...
\paragraph{Parametric Data Type}\label{sec:parametric_datatype}\hfill
...
\paragraph{Constrained Type}\label{sec:constrained_type}\hfill
...
\subparagraph{Functional Dependency}\label{sec:functional_dependency}\hfill
...
\paragraph{Variance}\label{sec:type_variance}\hfill
...
\subparagraph{Predicate Subtype}\label{sec:predicate_subtype}\hfill



\paragraph{Behavioral Subtype}\label{sec:behavioral_subtype}\hfill
...
\paragraph{Polymorphic Recursion}\label{sec:polymorphic_recursion}\hfill
...
\paragraph{Free Theorem}\label{sec:free_theorem}\hfill



\subsubsection{Value Polymorphism}\label{sec:value_polymorphism}
...
\paragraph{Exponential Type}\label{sec:exponential_type}\hfill
...
\paragraph{Associated Type}\label{sec:associated_type}\hfill
...
\begin{enumerate}
  \item Unit ($\mathsf{return}$) of the Monad -- Labelled with the
    Empty Effect
  \item Bind Operation ($T^\sigma\tau \mathsf{>>=} (\tau \rightarrow
...
Effect-grade Monad:
\begin{flalign*}
  & \mono{class}\;\mathsf{Effect}
    (m : \text{\emph{ef}} \rightarrow * \rightarrow *)
    \;\mono{where} & \\
  & \quad\quad\mono{type}\;\mathsf{Unit}\;m : \text{\emph{ef}} \\
  & \quad\quad\mono{type}\;\mathsf{Plus}\;m\;(f : \text{\emph{ef}})
    \;(g : \text{\emph{ef}}) : \text{\emph{ef}} \\
  & \quad\quad\mathsf{return} : a \rightarrow
    m\;(\mathsf{Unit\;m})\;a \\
  & \quad\quad(\mathsf{>>=}) : m\;f\;a \rightarrow
    (a \rightarrow m\;g\;b) \rightarrow m\;(\mathsf{Plus}\;m\;f\;g)\;b
\end{flalign*}

$\text{\emph{ef}}$ -- Domain, \emph{Kind}; Set of Effect Annotations
$\class{F}$
...
$\mathsf{(>>=)}$ -- Sequential Composition of Effectful Computations;
...
Session Calculus (\S\ref{sec:session_calculus})

\fist Cf. Session Calculus Embedding into FPCF \S\ref{sec:fpcf})
...
\emph{Session Effects} are Modelled by the $\mathsf{Session}$
Datatype:
\begin{align*}
  \mono{data}\;\mathsf{Session}
    ...
    \mid\; & \mathsf{Session}\; :+ \;\mathsf{Session}
...
Explicit Subtyping $sub : \mathsf{Sub} f g => m f a -> m g a$
...
with a notion of \emph{Session Effect}: Session Types and
\emph{Causal} (Non-commutative) Effects have the same structure--both
give an Ordered Analysis of the Operational Behavior of a Program.
...
\fist See Graded Monad (\S\ref{sec:graded_monad}) for an example of
Session Effects Implementation
