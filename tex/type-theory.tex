...
\paragraph{Algebraic Effect}\label{sec:algebraic_effect}
\hfill \\

alternative to using Monads (\S\ref{sec:monad}) to Model Side-effects

subsumed by Monads

\cite{nielson-nielson99}

``Lazy State'' and ``Lazy Writer'' cannot be captured by any Effect
System (E. Kmett)
...
\subparagraph{Effect Handler}\label{sec:effect_handler}
\hfill \\
...
\subparagraph{Extensible Effect}\label{sec:extensible_effect}
\hfill \\
...
\subparagraph{Communication Effect}\label{sec:communication_effect}
\hfill \\
...
\rightarrow A$ where $B$ is a Type Depending on a Variable of Type
$A$:
\[
  x : A \vdash B(x):\mathrm{Types}
\]
where $B(x)$ is the Fiber (\S\ref{sec:fiber}) of $p$ over $x:A$.
...
The \emph{Dependent Product Type} (or \emph{$\Pi$-type}) is the Type
of a Dependent Function. By Curry-Howard, $\Pi$-types model
Implication and Universal Quantification.

Generalizes the idea of a Polymorphic Function.

For a Type $A : \class{U}$ in Type Universe $\class{U}$, a Family
of Types:
\[
