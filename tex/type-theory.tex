...
\emph{Generic Datatype} (\S\ref{sec:parametric_datatype})
...
Weak form of Abstract Data Type (\S\ref{sec:abstract_datatype})

A Type that is not Parametric may be called a \emph{Concrete Data
  Type}
...
Indexed Families (Bundle or Fibration), represented by a \emph{Display
  Map} which is a Morphism $p : B \rightarrow A$.
% FIXME display map section?
...
Derivation (\emph{Subject Reduction}, \emph{Type Soundness})
\cite{wadler14}
...
Internal Language (\S\ref{sec:internal_logic}) of Closed
(\S\ref{sec:closed_monoidal}) Symmetric
(\S\ref{sec:symmetric_monoidal}) Monoidal Categories
