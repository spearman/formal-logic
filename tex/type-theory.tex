...
Product Type (\S\ref{sec:pi_type}) $\Pi_(a:A)B$ with a
Constant Codomain Type $B$:
\[
...
\subsubsection{$\Pi$-type}\label{sec:pi_type}

The \emph{$\Pi$-type} (or \emph{Dependent Product Type}) is the Type
...
\subsubsection{$\Sigma$-type}\label{sec:sigma_type}
