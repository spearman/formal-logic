...
Linear Logic (\S\ref{sec:classical_linear_logic})
...
(\S\ref{sec:constrained_type}) for a Freyd Categories
(\S\ref{sec:freyd_category}), i.e. Premonoidal Categories
(\S\ref{sec:premonoidal_category}) with a Cartesian Center (???), with
a Premonoidal Trace (\S\ref{sec:category_trace}) -- defines the
Dataflow Structure but lacks Temporal Properties \cite{jeffrey12}
%FIXME
...
Stateless Function, Causal Function, Decoupled Function: expression as
LTL Operators \cite{jeffrey12}

LTL Formulae: Reactive Types (\S\ref{sec:reactive_type})

Implementation in a Dependent Type Theory (\S\ref{sec:dependent_type})



% --------------------------------------------------------------------
\subsection{Reactive Type}\label{sec:reactive_type}
% --------------------------------------------------------------------

\cite{jeffrey12}:

Propositions of Linear Temporal Logic (\S\ref{sec:linear_temporal})
...
\begin{itemize}
  \item $\bigcirc \phi$ -- Next
  \item $\phi U \psi$ -- Until
\end{itemize}
...
\begin{itemize}
  ...
  \item $\phi \underline{U} \psi$ -- Non-strict Until
...
Duals of $\bigcirc$, $\underline{U}$, $\lozenge$, $\square$:
\begin{itemize}
  \item $\ominus \phi$ -- Yesterday: $\phi$ True at Time $s$ when True
    at Time $s - 1$
  \item $\phi \underline{S} \psi$ -- Non-strict Since
...
\begin{itemize}
  \item $A \Rightarrow B$ -- \emph{Stateless Functions}: Output Value
    at Time $t$ depends only on Input Value at Time $t$
...
Comonad Structure of $\square$: $extend$, $extract$, $duplicate$
...
$\bigcirc$, $\ominus$: Applicative Functors
(\S\ref{sec:applicative_functor})

$\lozenge$, $\blacklozenge$: Applicative Monads
...
(\S\ref{sec:cartesian_closed}): in $\cat{RSet}$, $\underline{S}$ (with
its Arguments flipped) is Left-adjoint to $\unrhd$ so $\cat{RSet}$ has
a Closed Structure for $\unrhd$ even if $\unrhd\cat{RSet}$ does not.
...
$C$ -- Chop Modality on Paths %FIXME
...
assuming a Discrete Time Model
...
\subsubsection{Signal Function}\label{sec:signal_function}
...
% --------------------------------------------------------------------
\subsection{Event}\label{sec:frp_event}
% --------------------------------------------------------------------
