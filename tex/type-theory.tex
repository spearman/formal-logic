...
\HandRight\; The paper \emph{Handlers of Algebraic Effects}
...
Algebraic Operations (\emph{Effect Constructors}) and Effect Handlers
(\emph{Effect Destructors}) are Dual (cf. Filinski: Reflection and
Reification) %FIXME

Handling a Computation is Composing it with a Unique Homomorphism
(defined Universally) with Domain a Free Model of the Algebraic Theory
of the Effects ``at hand'', and Range the Programmer-defined Model
(???) of the Algebraic Theory, Extending a Programmer-defined Map on
Values.
...
\begin{itemize}
  ...
  \end{itemize}
  \item Function Symbols $f : (\beta_1, \ldots, \beta_n) \rightarrow
    \beta$
  \item Relation Symbols $R : (\beta_1, \ldots, \beta_n)$
\end{itemize}
...
\emph{Base Terms}:
\[
  v ::= x \quad | \quad f (v_1, \ldots, v_n)
\]

\emph{Base Formulas} $\varphi$: Equations $v_1 = v_2$, Relation
Symbols $R (v_1, \ldots, v_n)$, Logical Connectives, Quantifiers
...
\begin{itemize}
  ...
  \item Maps $\llbracket f \rrbracket : \llbracket \beta_1, \ldots,
    \beta_n \rrbracket \rightarrow \llbracket \beta \rrbracket$ for
    each Function Symbol
  \item Subsets $\llbracket R \rrbracket \subseteq \llbracket \beta_1,
    \ldots, \beta_n \rrbracket$ for each Relation Symbol
...
\end{itemize}
where $\llbracket \beta_1, \ldots, \beta_n \rrbracket = \llbracket
\beta_1 \rrbracket \times \cdots \times \llbracket \beta_n \rrbracket$

\emph{Effect Signature} $\Sigma_{\mathrm{eff}}$:
\begin{itemize}
  \item Operation Symbols $\mono{op} : \beta_1, \ldots, \beta_m;
    A_1, \ldots, A_n$ where $\beta_1, \ldots, \beta_m$ are Base Type
    Parameters and $A_1, \ldots, A_n$ are Lists of Argument Arity
    Types (``;'' is ommitted if the List of Base Types is Empty and
    $n$ is used if all $A_i$ Lists of Argument Arities are Empty)
\end{itemize}
...
\emph{Effect Terms}:
\[
  T ::= w(v_1, \ldots, v_n) \quad | \quad
  \mono{op}_{v_1, \ldots, v_m} (X_1:A_1.T_1, \ldots, X_n:A_n.T_n)
\]
where $X_i:A_i$ are Lists of Base Terms of Arity Types.

$\Delta$ is a Set of Effect Variables $w:(\alpha_1, \ldots, \alpha_n)$
with rules:
\begin{enumerate}
  \item $(w:(\alpha_1, \ldots, \alpha_n) \in \Delta)$:
  \[
    {\Gamma \vdash v_1:\alpha_1, \ldots, v_n:\alpha_n}
    \over{\Gamma;\Delta \vdash w(v_1, \ldots, v_n)}
  \]
  \item $(\mono{op} : \beta_1, \ldots, \beta_m; A_1, \ldots, A_n
    \in \Sigma_\mathrm{eff})$:
    \[
      {\Gamma \vdash v_1:\beta_1, \ldots, v_m:\beta_m \quad\quad
        \Gamma, X_i:A_i; \Delta \vdash T_i \quad (i = 1, \ldots, n)}
      \over{\Gamma; \Delta \vdash
        \mono{op}_{v_1,\ldots,v_m}(X_i:A_i.T_i)_i}
    \]
\end{enumerate}
...
Theory:
\begin{itemize}
  \item For each Operator $\mono{op}:\beta_1, \ldots, \beta_m; A_1,
    \ldots, A_n \in \Sigma_\mathrm{eff}$ and $b_1, \ldots, b_m \in
    \llbracket \beta_1, \beta_2, \ldots \rrbracket$ take an Operation
    Symbol $\mono{op}_{b_1, \ldots, b_m}$ of (Countable) Arity
    $\sum_{i=1}^n|\llbracket A_i \rrbracket|$ (??? is this indexing
    correct) %FIXME
  \item Each Effect Term $\Gamma; \Delta \vdash T$ and each $c_1,
    \ldots, c_n \in \llbracket \Gamma \rrbracket$ gives rise to a Term
    $\Delta' \vdash T_{c_1, \ldots, c_n}$ with $\Delta'$ consisting of
    Effect Variables $w_{a_1, \ldots, a_m}$ for each $w:(\alpha_1,
    \ldots, \alpha_m) \in \Delta$ and $a_1, \ldots, a_m \in \llbracket
    \alpha_1, \alpha_2, \ldots \rrbracket$ (??? again indexing) %FIXME
  \item Equations:
    \[
      \Delta' \vdash T_{c_1, \ldots, c_n} = T_{c_1', \ldots, c_n'}
    \]
    (??? indexes) for any Conditional Equations $\Gamma; \Delta \vdash
    T = T'(\varphi)$ in $\thy{E}$ and any $c_1, \ldots, c_n \in
...
\begin{itemize}
  \item Set $M$
  \item Family of Maps $\{ \mono{op}_M : \llbracket \beta_1, \ldots,
    \beta_m \rrbracket \times \prod_{i=1}^n M^{\llbracket A_i
      \rrbracket} \rightarrow M \}_{\mono{op}:\beta_1, \ldots,
      \beta_m; A_1, \ldots, A_n \in \Sigma_{\mathrm{eff}}}$
\end{itemize}
such that the corresponding Maps $\mono{op}_M(b_1, \ldots, b_m, -)$
(where $b_1, \ldots, \b_m \in \llbracket \beta_1, \beta_2, \ldots
\rrbracket$) satisfy the Equations of the Induced Infinitary Effect
Theory.

A Homomorphism between Models $M$ and $N$ is a Map $f : M \rightarrow
N$ such that $\mono{op}_N \circ (id_{\llbracket \beta_1, \ldots,
  \beta_m \rrbracket} \times \prod_{i=1}^n f^{\llbracket A_i
  \rrbracket}) = f \circ \mono{op}_M$
...
\emph{Handler Types}:
\begin{flalign*}
  \quad \chi &::= X \quad | \quad F \sigma \quad | \quad 1 \quad
    | \quad \chi_1 \times \chi_2 \quad | \quad \sigma \rightarrow \chi &
\end{flalign*}

\emph{Handler Terms}:
\begin{flalign*}
  \quad h ::=& \; z(v_1, v_2, \ldots) \quad
    | \quad \mono{op}_{v_1, v_2, \ldots}
      (X_1:A_1.h_1, X_2:A_2.h_2, \ldots) & \\
    |&\quad \mono{if}\; \varphi \;\mono{then}\; h_1
      \;\mono{else}\; h_2 \quad | \quad \mono{return}\; u \quad
    | \quad \mono{let}\; x \;\mono{be}\; h \;\mono{in}\; h' & \\
    |&\quad \star \quad
    | \quad \langle h_1, h_2 \rangle \quad | \quad \mono{fst}\; h \quad
    | \quad \mono{snd}\; h \quad | \quad \lambda x:\sigma.h \quad
    | \quad h u &
\end{flalign*}
...
\begin{flalign*}
  \quad \underline{\tau} &::= F \sigma \quad | \quad 1 \quad
    | \quad \underline{\tau}_1 \times \underline{\tau}_2 \quad
    | \quad \sigma \rightarrow \underline{\tau} &
\end{flalign*}

\emph{Computation Term}:
\begin{flalign*}
  \quad t ::=& \; \mono{op}_{v_1, v_2, \ldots}
      (X_1:A_1.t_1, X_2:A_2.t_2, \ldots) & \\
    |&\quad \mono{if}\; \varphi \;\mono{then}\; t_1
      \;\mono{else}\; t_2 \quad | \quad \mono{return}\; u \quad
    | \quad \mono{let}\; x \;\mono{be}\; t \;\mono{in}\; t' & \\
    |&\quad \mono{try}\; t \;\mono{with}\; H(u_1,u_2,\ldots;
      t_1,t_2,\ldots) \;\mono{as}\; x \;\mono{in}\; t' \\
    |&\quad \star \quad | \quad \langle t_1, t_2 \rangle \quad
    | \quad \mono{fst}\; t \quad
    | \quad \mono{snd}\; t \quad | \quad \lambda x:\sigma.t \quad
    | \quad t u &
\end{flalign*}

\HandRight\; Note that Computation Types mirror Handler Types (except
...
\[
  ...
  H(u_1, u_2, \ldots; t_1, t_2, \ldots)
\]
can be used in place of:
\[
    \mono{try}\; t \;\mono{with}\; H(u_1,u_2,\ldots;
      t_1,t_2,\ldots) \;\mono{as}\; x \;\mono{in\;return}\; x
\]

When the Handler Signature consists of a single Handler Symbol $H$ it
may be ommitted:
\[
    \mono{try}\; t \;\mono{with}\; u_1,u_2,\ldots;
      t_1,t_2,\ldots \;\mono{as}\; x \;\mono{in}\; t'
\]
...
\begin{flalign*}
  \quad v ::=&\quad x \quad | \quad \true \quad | \quad \false \quad
      ...
      \quad |&\quad \mono{handler}\; \{ \mono{return}\; x
...
\emph{Computations}:
\begin{flalign*}
  \quad c ::=&\quad \mono{return}\; v \quad | \quad \mono{op}(v;y.c) \quad
            | \quad \mono{do}\; x \leftarrow c_1 \;\mono{in}\; c_2 &\\
      \quad |&\quad \mono{if}\; v \;\mono{then}\; c_1
              \;\mono{else}\; c_2 \quad
            | \quad v_1 v_2 \quad | \quad \;\mono{with}\; v
              \;\mono{handle}\; c&
\end{flalign*}
