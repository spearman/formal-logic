...
Isorecursive (\S\ref{sec:isorecursive_datatype}): $roll$ and $unroll$
are Inverses

Equirecursive (\S\ref{sec:equirecursive_datatype}): $unroll$ is the
Identity
...
\fist Note the following uses a Haskell-like notation
...
\begin{flalign*}
  & \mono{class}\;\mathsf{Effect}
    ...
    \;\mono{where} & \\
  & \quad\quad\mono{type}\;\mathsf{Unit}\;m : \eff \\
  & \quad\quad\mono{type}\;\mathsf{Plus}\;m\;(f : \eff)
    \;(g : \eff) : \eff \\
  & \quad\quad\mathsf{return} : a \rightarrow
    m\;(\mathsf{Unit\;m})\;a \\
  & \quad\quad(\bindop) : m\;f\;a \rightarrow
...
(\S\ref{sec:equirecursive_datatype}) so the Implementation restricts
