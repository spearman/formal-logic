...
(\S\ref{sec:formal_system}) where each Term (\S\ref{sec:term}) has a \emph{Type}
(\S\ref{sec:type}) and Operations are restricted to Terms of specific Types.

A Type Theory may have two Signatures: $\sigma_\tau$ for Type Symbols and
$\sigma_o$ for Object (Term) Symbols. From this one may construct a Monad $R =
R_{\sigma_o}$, a Module over $R$: $LM = LM_{\sigma_\tau,\sigma_o}$, and a
Contextual Category (\S\ref{sec:contextual_category}) $CC(R,LM)$ describing a
Class of Sub-quotients of $CC(R,LM)$ in terms of Objects Constructed from $R$
and $LM$. \cite{voevedsky14}

For Multi-sorted Type Theory see Pure Type Systems (\S\ref{sec:pts})
...
Language (or \emph{Calculus}) for Category Theory (Part
\ref{part:category_theory}), and conversely one may think of Category Theory as
providing Semantics for Type Theory

As a Formal Theory (\S\ref{sec:formal_theory}), Judgements
(\S\ref{sec:judgement}) in Type Theory are of three kinds\cite{hott13}:
\begin{enumerate}
...
\end{enumerate}
with a Deductive Apparatus (\S\ref{sec:deductive_apparatus}) consisting of
Inference Rules (\S\ref{sec:type_inference}) only and no Axioms. Judgemental
Equality is an Equivalence Relation respected by Typing.

every Type Theory gives rise to a Cartesian Multicategory
(\S\ref{sec:cartesian_multicategory}) with Objects as Types and Multimorphisms
as Terms

<https://twitter.com/davidad/status/1216565760954458113>:

a Type Theory corresponds to a Class of Category (e.g. Cartesian Closed)

in a Type Theory with user-definable Inductive Types, each Inductive adds its
own Deduction Rules, either explicitly (as in HoTT) or implicitly via Axiom
Schemes indexed by Type Declarations (as in pCuIC)
...
2019 - Krishnaswami -
\emph{On the Relationship Between Static Analysis and Type Theory}
-
\url{https://semantic-domain.blogspot.com/2019/08/on-relationship-between-static-analysis.html}
...
-- $3$-theory of Cartesian Monoidal $2$-categories (actually Cartesian
...
\url{https://golem.ph.utexas.edu/category/2020/02/type_theory_and_propositions.html}:

Propositions as a \emph{Kind of Types}

Propositions as \emph{some Types}:
\url{https://ncatlab.org/nlab/show/propositions+as+types#PropositionsAsSomeTypes}:

Propositions are identified with \emph{particular} Types, but not \emph{all}
Types are regarded as Propositions: \emph{Propositions} are Types with
\emph{at most one Term}-- paradigm found in Internal Logic of Categories (e.g.
Toposes, HoTT)
...
Inhabitant (cf. HoTT h-level 1)

in Propositions as Types, all ``Type-theoretic Operations'' represent
corresponding \emph{Logical Operations}

in the Propositions as Types paradigm, all ``Type-theoretic Operations''
represent corresponding Logical Operations (\S\ref{sec:logical_connective})--
Dependent Sum is Existential Quantification, Dependent Product is Universal
Quantification, etc.
...
\fist cf. Statistical Data Type (\S\ref{sec:statistical_data_type})



\subsubsection{Replicated Data Type}\label{sec:replicated_data_type}

\paragraph{Conflict-free Replicated Data Type (CRDT)}\label{sec:crdt}\hfill

\subparagraph{Commutative Replicated Data Type (CmRDT)}\label{sec:cmrdt}\hfill
...
\subsubsection{Bracket Type}\label{sec:bracket_type}
...
\url{https://golem.ph.utexas.edu/category/2022/07/identity_types_in_context.html}:
...
Model of Type Theory in Kan Complexes (Voevodsky & Sreicher 2006/2009) allows
one to interperet ``Types as Topological Spaces''
...
\emph{Differentiation} -- the \emph{Derivative} of a Data Type $D$ is the
Type of $D$-structures with a single ``hole'', i.e. a distinguished location not
containing any Data;
Satisfies the same Algebraic Laws as Differentiation (\S\ref{sec:derivative}) in
Calculus
...
Recursive Sum Types
(\url{https://lean-lang.org/functional_programming_in_lean/getting-to-know/datatypes-and-patterns.html})
...
a Forest (\S\ref{sec:forest_type}) is a Mutually Recursive Definition of a Tree
(\S\ref{sec:tree_type})

\fist cf. Mutually Recursive Function System (\S\ref{sec:mrfs})



\subsubsection{Guarded Recursive Data Type}\label{sec:grdt}
...
\emph{Coinductive Type} (\emph{Codata}), Coinduction (\S\ref{sec:coinduction})

Terminal Coalgebras (\S\ref{sec:terminal_coalgebra})

Greatest Fixed-points (\S\ref{sec:least_fixedpoint})

Calculus of (Co)inductive Constructions (\S\ref{sec:coinductive_calculus})
...
\emph{Generic Type} (also \emph{Parameterized} or \emph{Parametric Type})
...
% ------------------------------------------------------------------------------
\subsection{Logically Qualified Data Type}\label{sec:logically_qualified}
% ------------------------------------------------------------------------------

\cite{rondon-kawaguchi-jhala08}

\emph{Liquid Type} or \emph{Pattern Type}
...
Datatypes and Definitions that take \emph{Types} as arguments
(\url{https://lean-lang.org/functional_programming_in_lean/getting-to-know/polymorphism.html})
...
\emph{Parametricity}: Abstract Uniformity Property; captures the intuition that
all instances of a Polymorphic Function (\S\ref{sec:polymorphic_function})
behave the same way for all Types (``for all''); used to define Natural
Transformations (\S\ref{sec:natural_transformation}) in Programming Languages

Parametric (Generic) Type (\S\ref{sec:generic_type}): a weak form of Abstract
Type (\S\ref{sec:abstract_type})
...
\[
  x:\tau \vdash id(x):\tau
\]
where the Type of the Term $id(x)$ is $\tau$, which is dependent on the Type
$\tau$ of $x$, where the Type of $id$:
\[
...
having Pattern Matching on Types in a Type System would break Parametricity
(idris stackoverflow user Xash) %FIXME link

Type-level Function Abstraction $\Lambda$ (\S\ref{sec:type_operator})

Second-order $\lambda$-calculus (System F \S\ref{sec:system_f})

Bounded Polymorphism (\S\ref{sec:bounded_polymorphism}): Bounded Quantification;
Polymorphism restricted to Subtypes
...
\subsubsection{Predicative Polymorphism}\label{sec:predicative_polymorphism}

Type $\tau$ containing Type Variable $\alpha$ may not be used such that $\alpha$
is instantiated to a Polymorphic Type.

Martin-L\"of Type Theory, NuPRL



\subsubsection{Impredicative Polymorphism}\label{sec:impredicative_polymorphism}
...
Systems and their Parts are known only by the Types of Behavior they can
exhibit; relationships between Parts of a Whole are considered in terms of how
\emph{Behavioral Constraints} are passed between them; \emph{Inter-modal Logic}
(\S\ref{sec:intermodal_logic}) generalizing usual Alethic Modalities in the
setting of ''Symmetric Accessibility''

Observational Equivalence -- Bisimulation in Coalgebra

(cf. Observational Type Theory \S\ref{sec:observational_type})

associates \emph{Parts} with (Regular) \emph{Epimorphisms}
(\S\ref{sec:epimorphism})

Lattice of Parts

Behaviorally, Parts are ``measurement devices'' by which Observations are made

\emph{Compatibility} of Parts within a Whole; has a Right Adjoint called
\emph{Ensurance} corresponding to a \emph{Determination Relation}
...
H\"uttel et al. 2016 --
\emph{Foundations of Session Types and Behavioural Contracts}
...
% ------------------------------------------------------------------------------
\subsection{Spatial Behavioral Type}\label{sec:spatial_behavioral}
% ------------------------------------------------------------------------------

Caires04 - \emph{Behavioral and Spatial Observations in a Logic for the
  Pi-calculus} -- $\pi$-calculus Logic based on purely structural
Spatial and Behavior Observations, with ``purely structural'' meaning
\emph{Observations} that can be determined by inspection of the local structure
of the Processes, and therefore not including Adjunct Operators: there are some
Formulae that are Adjoint to the Parallel Type that are Provably Undecidable
...
constraints of Realizability (\S\ref{sec:realizability}) and Curry-Howard forces
a Canonical Semantics (RCast 33
...
a Dependent Type is actually a Kind (\S\ref{sec:kind}), i.e. its ``members'' are
Types

A Dependent Product Type (\S\ref{sec:pi_type}) is defined by a Dependent
Function (\S\ref{sec:dependent_function}).

Logical Quantifiers (\S\ref{sec:quantifier})

Intuitionistic Type Theory (\S\ref{sec:intuitionistic_type}), Unified Theory of
Dependent Types (UTT \S\ref{sec:utt})
...
2019 - Chang, Ballantyne, Turner, Bowman -
\emph{Dependent Type Systems as Macros}
...
A \emph{Dependent Function} is a Function from Terms to Types (Codomain varies
depending on Argument). The Type of a Dependent Function is a \emph{Dependent
Product Type} (\S\ref{sec:pi_type}).
...
The \emph{$\Pi$-type} (or \emph{Dependent Product Type} or \emph{Dependent
Function Type}) is the Type of a Dependent Function, i.e. a Dependent Function
Space. By Curry-Howard, $\Pi$-types model Implication and Universal
Quantification.
...
\]
can be defined by a Dependent Function which assigns a Type $B(a) : \class{U}$
to Each Term $a : A$. The Type of this Function is a Dependent Product Type:
\[
  (\Pi x:A)B(x)
\]

If $B$ is Constant, then $(\Pi x:A)B$ is Judgementally Equal to the Function
Type (\S\ref{sec:function_type}) $A \rightarrow B$:
\[
...
A Polymorphic Function returning elements of Type $C$ has Polymorphic Type
...
Delimited Continuations (\S\ref{sec:delimited_continuation}) arise as
...
2017 - Chang, Knauth, Greenman - \emph{Type Systems as Macros}
...
Formalizing Session Types in Coq:
\url{https://github.com/umazalakain/session-types-coq}
...
2019 - Fowler - \emph{Typed Concurrent Functional Programming with Channels,
  Actors, and Sessions}
...
% ------------------------------------------------------------------------------
\subsection{Dynamic Typing}\label{sec:dynamic_typing}
% ------------------------------------------------------------------------------
...
Semantics for Moggi's Computational $\lambda$-calculus in terms of (Strong)
Indexed Monads, giving a one-to-one correspondence between Indices of the
Denotations and the Effect Annotations of traditional Effect Systems

Dual: Indexed Comonads give a ``unified'' Semantics and Effect System to
Coeffects (\S\ref{sec:coeffect}), i.e. Contextual notion of Effects

Wadler-Thiemann03 -- Syntactic correspondence between Type-and-effect Systems
and the Monadic Semantics by Annotating Monadic Type Constructors with Effect
Sets of the Type-and-effect System
...
in a base Language without State, Exceptions cannot express Continuations
...
may not be the case and State may be expressable
...
$\mono{handle}\;e\;M\;N$ -- $M$ acts as Handler for Raising of $e$ in the
Evaluation of $N$

An Uncaught Exception $\mono{raise}\;e\;V$ without a ``Dynamically Enclosing
Handler'' has no Reduction

An Exception Name $e$ can be passed like any other Value

An Exception Handler (an Evaluation Context, i.e. as a Continuation) cannot be
made into a Value
...
an Exception is a form of ``jump'' whose target is determined Dynamically %FIXME
...
\]
where $\{\cdots\}_{e\in\mathbf{exc}}$ is a Set of Computations, one for each
Exception $e \in \mathbf{exc}$

carries out $M$, ``Intercepting'' Exceptions $e \in \mathbf{exc}$ by carrying
out predefined Computations $M_e \in A + \mathbf{exc}$ instead
...
the Computations $M_e$ give a new Model $\mathcal{M}$ for the Theory of
Exceptions with same Carrier (i.e. Domain \S\ref{sec:domain}) $A + \mathbf{exc}$
and for each $e$, each $\mono{raise}_e ()$ is instead Interpreted by $M_e$

\[
  h(M) \defeq M \;\mono{handled\;with}\;
    \{\mono{raise}_e \mapsto M_e\}_{e \in \mathbf{exc}}
\]
is a Unique Homomorphism (Preserving Operations) from $A + \mathbf{exc}$ to
$\mathcal{M}$ that Extends $in_1 : A \rightarrow A + \mathbf{exc}$, i.e. such
that:
\[
...
\fist The paper \emph{Handlers of Algebraic Effects} \cite{plotkin-pretnar09}
(Plotkin, Pretnar 2009) is a preliminary version of
\emph{Handling Algebraic Effects} \cite{plotkin-pretnar13} (Plotkin, Pretnar
2013). In \cite{plotkin-pretnar09}, there are restricted facilities for defining
Handlers (the \emph{Minimal} approach). Two levels of Language are considered:
in the first there are no Handlers but it is used to define Handlers, and if
they give Models then they are used in Handling Constructs in the second level
(with no facilities for further definition of Handlers). In
\cite{plotkin-pretnar13} Handlers and Handling may be nested arbitrarily deeply
(the \emph{Maximal} approach).
...
Algebraic Operations \S\ref{sec:algebraic_operation}
(\emph{Effect Constructors}) and Effect Handlers \S\ref{sec:effect_handler}
...
Algebraic Operations \S\ref{sec:algebraic_operation} (\emph{Effect
  Constructors}: giving rise to the Effects) are Dual to Effect Handlers
\S\ref{sec:effect_handler} (\emph{Effect Deconstructors}: proceeding according
to Effects already created)
...
$\ulcorner\phi\urcorner$ are Names, so they are \emph{Namespaces}
...
\emph{Simply-typed $\lambda$-calculus} ($\lambda^\rightarrow$) is a Typed
Interpretation of $\lambda$-calculus with one Type Constructor
(\S\ref{sec:type_constructor}) for Function Types (\S\ref{sec:function_type}):
$\rightarrow$

Strongly Normalizing, not Turing Complete: addition of $fix_\alpha$ Operator or
Recursive Types (\S\ref{sec:inductive_type}) gives Turing Completeness

``Simply-typed'' is to distinguish from $\lambda$-cube (\S\ref{sec:lambda_cube})
Extensions and other variations such as:
\begin{itemize}
...
2016 - Downen, Maurer, Ariola, Jones - Typed $\lambda$-calculus arises
...
% ------------------------------------------------------------------------------
\subsection{Differential $\lambda$-calculus}\label{sec:differential_lambda}
% ------------------------------------------------------------------------------
...
account for the Finite nature of Computation; cf. ``mainstream'' Mathematical
viewpoint where Functions are Total, defined on Vector Spaces (rather than
Ordered Sets), and Approximated by Polynomials rather than by Finite Functions;
in Analysis, Functions are Approximated by (Multi)linear Maps through (Iterated)
Differentiation

\fist Differential Category (\S\ref{sec:differential_category})
...
1988 - Coquand, Huet - \emph{The Calculus of Constructions}

$\lambda\Pi\omega$ -- Higher-order Dependently-typed $\lambda$-calculus

$\mathtt{Prop}$ is \emph{Impredicative} (\S\ref{sec:impredicative_definition});
cf. Intuitionistic Type Theory (\S\ref{sec:intuitionistic_type}) which is
\emph{Predicative}
...
Propositinal Extensionality, Proof Irrelevance

\url{https://cstheory.stackexchange.com/questions/7561/whats-the-relation-and-difference-between-calculus-of-inductive-constructions-a/30662}:

Intuitionistic Type Theory is a System of Dependent Types, Universes, Inductive
Families

\url{https://cstheory.stackexchange.com/questions/36043/impredicative-in-type-theory}:

example of an Impredicative Definition that is \emph{Circular} but not known to
be \emph{Contradictory} is that of the Natural Numbers in Set Theory:
\[
  \nats \defeq \cap\{ X | X \text{is an Inductive Set} \}
\]
where a Set is an Inductive Set if it contains $\varnothing$ and is Closed under
the Set-theoretic Successor Operation; $\nats$ is itself Inductive

an example of an Impredicative Definition that leads to an Inconsistency is that
of the MLTT71 that included an Axiom that stipulates a Type of all Types which
was shown to make the the Type Theory Inconsistent: in the Product Rule, one can
Conclude either:
\begin{enumerate}
  \item $\Gamma \vdash \forall x : A.B : Type$
  \item $\Gamma \vdash \forall x : A.B : BigType$
\end{enumerate}
(2.) is the \emph{Predicative} approach, resulting in a strictly
\emph{Stratified Type Theory} (effectively removing the Axiom of Reducibility),
while (1.) is ``\emph{Impredicative}'', since $Type : BigType$, if $B : Type$,
then $\forall X : Type.B : Type$ is an expression Quantifying over $Typ$
comprising the Product Type being defined as one of its Elements; with (1.), one
must renounce having ``\emph{Strong Elimination Rules}'' (TODO: clarify), i.e.
in Coq you cannot extract anything from a Proposition other than a Proof of some
other Proposition, and as a corollary you have no way to Prove that two Proofs
of the same Proposition are different from each other
...
\paragraph{Calculus of Inductive Constructions}\label{sec:cic}\hfill

adds Inductive Types (\S\ref{sec:inductive_type})
...
(\S\ref{sec:cumulative_hierarchy}) of Predicative (without self-referencing
definitions) Types of Types \emph{and} Impredicative (with self-referencing
definitions) Types of Propositions %FIXME

Intensional Dependent Type Theory



\subparagraph{Predicative Calculus of Inductive Constructions}
\label{sec:pcic}\hfill

removes some Impredicativity (\S\ref{sec:impredicative_definition})



\paragraph{Calculus of (Co)inductive Constructions}
\label{sec:coinductive_calculus}\hfill

adds Co-induction (\S\ref{sec:coinductive_type})
...
\paragraph{$\mu$GV}\label{sec:mu_gv}\hfill

2016 - Lindley, Morris - \emph{Talking Bananas: Structural Recursion for Session
Types}

$\mu$MALL -- Modal-$\mu$ (\S\ref{sec:modal_mu_logic}) Multiplicative Additive
Linear Logic (\S\ref{sec:mall}); Baelde12
...
\emph{(Dependent) Martin-L\"of Type Theory (MLTT)})

\emph{Predicativity} (\S\ref{sec:predicative_definition}); cf. Calculus of
Constructions (\S\ref{sec:coc}) which has Impredicative $\mathtt{Prop}$
...
NuPrl -- Martin-L\"of Type Theory is expressive but Type-checking requires more
Theorem Proving (Zhu-Xi05) %FIXME

Dependent Type Theory (\S\ref{sec:dependent_type_theory})

Inductive Types (\S\ref{sec:inductive_type})

Predicative (Parametric) Polymorphism (\S\ref{sec:predicative_polymorphism})

Modelled by Locally Cartesian Closed Category (\S\ref{sec:locally_cartesian})
...
\url{https://www.cs.bham.ac.uk/~mhe/HoTT-UF-in-Agda-Lecture-Notes/HoTT-UF-Agda.pdf}



% ------------------------------------------------------------------------------
\subsection{Unified Theory of Dependent Types (UTT)}\label{sec:utt}
% ------------------------------------------------------------------------------
...
% ==============================================================================
\section{Native Type Theory}\label{sec:native_type_theory}
% ==============================================================================

\url{https://golem.ph.utexas.edu/category/2021/02/native_type_theory.html}

Language $\rightarrow$ Category $\rightarrow$ Topos $\rightarrow$ Type System
...
Voevodsky - \emph{Univalence Principle} identifies \emph{Equivalent Types}
...
% ------------------------------------------------------------------------------
\subsection{Simplicial Type Theory}\label{sec:simplicial_type_theory}
% ------------------------------------------------------------------------------
...
\subsubsection{Semi-simplicial Type}\label{sec:semi_simplicial}

\url{https://golem.ph.utexas.edu/category/2024/03/semisimplicial_types_part_i_mo.html}
...
Granule: Statically-typed, Linear, Functional Language with Graded Modal
Types -- \url{https://github.com/granule-project/granule}
...
% ==============================================================================
\section{Univalent Type Theory}\label{sec:univalent_type_theory}
% ==============================================================================
...
\fist Homotopy Type Theory (\S\ref{sec:hott}): Synthetic development of Homotopy
Theory within Univalent Mathematics

Univalent Type Theory has a Model in Homotopy Types, and without considering
Models, Types ``behave like'' Homotopy Types, automatically
...
Synthetic development of Homotopy Theory (\S\ref{sec:homotopy}) within
Univalent Mathematics (\S\ref{sec:univalent_type_theory})

Univalent Type Theory has a Model in Homotopy Types
...
%FIXME: move under higher type theory ?
...
2015 - Shulman - \emph{Brouwer's Fixed-point Theoremin Real-cohesive Homotopy
  Type Theory}
