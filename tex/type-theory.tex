...
\fist cf. Behavioral Types \S\ref{sec:behavioral_types}
...
Calculus of Capabilities (\S\ref{sec:capabailities_calculus})
...
(note in Xi-Ren-Wu-Blair15 the Type Constructor based on Linear
Implication $\multimap$ is written $\rightarrow_l$; here we will just
use $\multimap$)
...
$\multimap$ -- Type Constructor based on Linear Implication; applying
a Function (of Viewtype) $\hat{T}_1 \multimap \hat{T}_2$ to a Value of
Viewtype $\hat{T}_1$ yields a result of Viewtype $\hat{T}_2$ and
\emph{consumes} the Function itself
...
\[
  \forall b : bool. \Bool(b)
    \multimap \Choose(\hat{T}_1, \hat{T}_2, b)
\]

Constant Function $\mono{thread\_create}$ and corresponding $c$-type:
\[
  \mono{thread\_create} : (\Unit \multimap \Unit) \Rightarrow \Unit
\]
...
most closely related to SILL (Monadic instead of Linear) %FIXME
