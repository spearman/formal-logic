...
that only one \emph{Reference} exists to a Value
...
Multi-party Asynchronous Session Types
(\S\ref{sec:multiparty_session}) -- Honda-Yoshida-Carbone08
...
2007 - Honda,Yoshida,Carbone - \emph{Multiparty Asynchronous Session
  Types}

Global Type (\S\ref{sec:global_type}) provides shared agreement among
Communication Peers
...
\begin{flalign*}
  ...
  \quad P ::=& \; \overline{a}[2 \ldots n](\tilde{s}).P \;
    | \; a[i](\tilde{s}).P & \\
  % value send/value receive
    |&\; s!\langle\tilde{e}\rangle;P \;|\; s?(\tilde{x});P & \\
  % session delegate/session receipt
...
\paragraph{Borrowed Type}\label{sec:borrowed_type}\hfill
...
\emph{Capabilities}: %FIXME xref, cite Pottier 07
...
Alias Types -- no Regions (Walker-Morrisett00) %FIXME
