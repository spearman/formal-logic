...
\emph{Effect Terms}:
\[
  T ::= w(\vec{v}) \quad | \quad \mono{op}_{\vec{v}}
...
\begin{flalign*}
  \quad t ::=& \; \mono{op}_{\vec{v}}
    ...
    |&\quad \mono{try}\; t \;\mono{with}\; H(\vec{u};
...
Function Symbols $f : \alpha \rightarrow \beta$

Operation $\mono{op} : \alpha \rightarrowtriangle \beta$

Effect Theory (\S\ref{sec:effect_theory}) $\thy{T}$
...
When $\alpha = 1$, $\mono{op}$ may be written $\mono{op}:\beta$

Value Types $A,B,\ldots$:
\begin{flalign*}
  \quad A,B ::=&\; b \;|\; 1 \;|\; A \times B \;|\; \sum_{\ell \in
    L}A_{\ell} \;|\; U \underline{C} &
\end{flalign*}

Computation Types $\underline{C},\ldots$:
\begin{flalign*}
...
Value Terms $V,W,\ldots$:
\begin{flalign*}
  \quad V,W ::=&\; x \;|\; \mono{f}(V) \;|\; \langle \rangle
    \;|\; \langle V,W \rangle \;|\; \ell (V) \;|\; \mono{thunk}\; M &
\end{flalign*}
%FIXME is \mono{f} an operation here?


Continuation Variables $k,\ldots$

Computation Terms $M,N,\ldots$:
\begin{flalign*}
  \quad M,N ::=&\; \mono{match}\; V \;\mono{with}\;
    ...
    \;|\; \lambda x:A.M \;|\; M V & \\
    \;|\;& \mono{op}_V(x:\beta.M) \;|\; k(V)s
    \;|\; M \;\mono{handled\;with}\; H \;\mono{to}\; x:A.N &
\end{flalign*}


Handler Terms $H,\ldots$:
\begin{flalign*}
  \quad H ::=&\; \{ \mono{op}_{x:\alpha}
    (k:\beta \rightarrow \underline{C}) \mapsto
    M_{\mono{op}} \}_{\mono{op:\alpha \rightarrow \beta}} &
\end{flalign*}
...
$\{\cdots\}_{\mono{op}: \alpha \rightarrowtriangle \beta}$ Set of
Computations, one for each Operation Symbol $\mono{op}: \alpha
...
\[
  \mono{op}_V(x:\beta.M)
\]
first triggers Operation $\mono{op}$ with Parameter $V$, then binds
...
\[
  \mono{op}_{x:\alpha}
    ...
    M_{\mono{op}}
\]
one for each Operation Symbol $\mono{op}$

Handling Terms $M_{\mono{op}}$ are dependent on their Parameters
