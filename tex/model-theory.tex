...
\emph{Domain} (\S\ref{sec:domain}) along with an \emph{Interpretation
  Function} (\S\ref{sec:interpretation}) $I$ that maps Symbols in the
Signature (\S\ref{sec:signature}) to their \emph{Realizations} in the
Structure.
...
\[
    \mathcal{A} = (A, I)
\]
with Domain $A$, Interpretation Function $I$.
...
In Model Theory an \emph{Interpretation} fixes the \emph{Domain of
  Discourse} for expressions of Symbolic Logic (Part
\ref{sec:formal_logic}); that is it assigns the Sets and Relations
that Variables can range over.
...
(\S\ref{sec:property}) of the Symbols and Strings of Symbols of the
Object Language.
...
For a Formula $\phi$ and a Structure $A$, a \emph{Satisfaction
  Relation} is denoted:
\[
    \mathcal{M} \vDash \phi
\]
...
$\phi$ is called a \emph{Model} of $\phi$: $A \vDash \phi$,
(equivalently ``$\phi$ is True in $A$'').
...
% --------------------------------------------------------------------
\subsection{Boolean Satisfiability Problem}\label{sec:boolean_satisfiability}
% --------------------------------------------------------------------
