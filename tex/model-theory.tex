...
A Valuation $v : L \rightarrow \{true, false\}$ may be denoted
$v(\phi) = [[\phi]]_v$ for a Formula $\phi \in L$.
...
For a Sentence $\phi$ and a Structure $A$, a \emph{Satisfaction
  Relation} is denoted:
\[
    A \vDash \phi
\]

If $\phi(\bar{x})$ is a Formula with Free Variables $\bar{x}$, and $A$
is a Structure with Elements $\bar{a}$ that gives a Closed Formula
under Substitution (\S\ref{sec:substitution}), then:
\[
    A \vDash \phi(\bar{x})[\bar{a}]
\]
...
Satisfiability of arbitrary First-order Formulas is Undecidable.
...
\subsubsection{Standard Model}\label{sec:standard_model}
